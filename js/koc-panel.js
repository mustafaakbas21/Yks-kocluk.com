...
import {
  ...
  loadEmails,
  wireZohoInbox,
  probeZohoMailConfigurationOnce,
...
import {
  ...
  renderDashboardKpis,
  setDashboardKpisLoading,
  configureDashboardCharts,
  fetchAndRenderAppointmentChart,
  renderAppointmentsChart,
  renderNetBasariChart,
  initDashboardMeetingActivityToggles,
  renderInstitutionNetTrendChart,
  renderDashboardAppointmentIntensityBar,
  configureDashboardInsights,
  renderDashboardInsightTable,
} from "./dashboard.js";
import {
  configureDashboardActivities,
  refreshDashboardMeetingActivity,
  getDashboardMeetingLogsSnapshot,
} from "./dashboard-activities.js";
import { initGorusmeOdasiCockpit } from "./gorusme-odasi-cockpit.js";
import { initMrCockpit, refreshMrIfActive } from "./mr-cockpit.js";
import { initCoachInboxChatView, destroyCoachInboxChatView } from "./coach-inbox-chat.js";
import { startPanelGlobalRealtime } from "./panel-global-realtime.js";
import {
  YKS_AVATAR_MALE_URLS,
...
import {
  ...
  dpFocusField,
  dpParseDecimalField,
} from "./dp-ui-feedback.js";

await getCoachPanelGate();

/** Aktif görünüme göre deneme/optik net kuralı (ÖSYM / Y3). */
function coachNetFromBranchDy(d, y) {
  var optikView = document.getElementById("view-deneme-analiz-optik");
  var daView = document.getElementById("view-deneme-analiz-denemeler");
  var el = null;
...
function showSaAnalyticsToolBanner() {
  ...
  if (tool !== "net-sihirbazi" && tool !== "yks-puan" && tool !== "tercih-sihirbazi") return;
...
/**
 * `students` / `exams` / `appointments`: sunucu tarafı `coach_id` filtresi (veri sızıntısı önleme).
 * Diğer koleksiyonlar: `filterSnapshotDocsByCoach` ile istemci süzgeci.
 */
...
/** Abonelik kurulamadığında ilk yükleme iskeletini temizle (sonsuz "Yükleniyor…" önleme) */
function showFirestoreSubscriptionBlockedMessage() {
  var msg =
    "Veriler yüklenemedi: koç kimliği çözülemedi (oturum / kullanıcı adı). Çıkış yapıp yeniden giriş deneyin veya profilinizde kullanıcı adı olduğundan emin olun.";
  var de = document.getElementById("denemeTableBody");
  if (de)
    de.innerHTML =
      '<tr><td colspan="6" class="table-empty table-empty--error">' + escapeHtml(msg) + "</td></tr>";
  var ex = document.getElementById("examsPageBody");
  if (ex)
    ex.innerHTML =
      '<tr><td colspan="6" class="table-empty table-empty--error">' + escapeHtml(msg) + "</td></tr>";
  var st = document.getElementById("activeStudentsList");
  if (st) st.innerHTML = '<li class="mini-list__empty">' + escapeHtml(msg) + "</li>";
  var pay = document.getElementById("paymentsTableBody");
  if (pay)
    pay.innerHTML =
      '<tr><td colspan="5" class="table-empty table-empty--error">' + escapeHtml(msg) + "</td></tr>";
  var cari = document.getElementById("muhasebeCariBody");
  if (cari)
    cari.innerHTML =
      '<tr><td colspan="7" class="table-empty table-empty--error">' + escapeHtml(msg) + "</td></tr>";
}
...
let firestoreUnsubs = [];
/** Appwrite Realtime kapatıcı; yeniden abonelikte temizlenir */
let stopPanelGlobalRealtime = null;
/** Realtime yokken veriyi canlı tutmak için getDocs aralığı (ms) */
const PANEL_FIRESTORE_POLL_MS = 90000;
...
let cachedAppointmentRawDocs = [];
let currentStudentDetailId = null;
/** Öğrenci detay Elite Analytics: hızlı tab geçişlerinde eski fetch sonuçlarını yok say */
let sdEliteAnalyticsSeq = 0;
...
var tmEditorPageOrder = [];
var tmEditorAnnotations = {};
...
var tmAiOverlaySteps = [
  {
    title: "Yapay Zeka havuzdan en uygun soruları seçiyor…",
    sub: "Filtrelerinize uygun adaylar taranıyor",
  },
  { title: "En iyi eşleşmeler belirleniyor…", sub: "Ders, konu ve zorluk kriterleri uygulanıyor" },
  { title: "Test tasarımına aktarılıyor…", sub: "Seçilen görseller sıraya alınıyor" },
];
  ...
  form.addEventListener("submit", async function (e) {
    ...
    try {
      while (selectedNewQuestions.length < n && attempts < maxAttempts) {
        ...
        var fetchCount =
          attempts === 1 ? Math.min(n + 8, 80) : Math.min(Math.max(remaining + 4, remaining), 80);
...
async function tmOpenFirestoreTestInAnnotator(testId) {
  ...
  try {
    ...
    if (currentView !== "testmaker") navigateTo("testmaker");
    await tmWsLoadPdfFromBuffer(buf);
    showToast("Test PDF Test Oluşturucu’da açıldı.");
  } catch (err) {
    console.error(err);
...
function tmOpenPdfEditorFromLib(lid) {
  ...
  tmLibGetFull(lid).then(function (rec) {
    ...
    tmWsLoadPdfFromBuffer(rec.buffer).then(function () {
      showToast("PDF Test Oluşturucu’da açıldı.");
    });
  });
}
...
function dispatchGoCockpitRefresh() {
  try {
    if (currentView === "gorusme-odasi") window.dispatchEvent(new CustomEvent("go-cockpit-refresh"));
  } catch (e) {}
}

/** Görüşme Odası → Test Oluşturucu: ders/konu ön dolumu */
function tmApplyGorusmeOdasiAiPrefill() {
  try {
    var raw = sessionStorage.getItem("gorusmeOdasiAiPrefill");
    if (!raw) return;
    sessionStorage.removeItem("gorusmeOdasiAiPrefill");
    var o = JSON.parse(raw);
    var courseEl = document.getElementById("tmWsCourse");
    var topicEl = document.getElementById("tmWsTopic");
    var titleEl = document.getElementById("tmWsTitle");
    var subjEl = document.getElementById("tmWsSubject");
    if (courseEl && o.subject) courseEl.value = String(o.subject);
    if (topicEl && (o.topic != null || o.display != null)) {
      topicEl.value = String(o.topic != null && o.topic !== "" ? o.topic : o.display || "");
    }
    if (titleEl && !String(titleEl.value || "").trim() && (o.display || o.topic)) {
      var examPart = o.exam ? String(o.exam) + " — " : "";
      titleEl.value = (examPart + (o.display || o.topic || "")).slice(0, 200);
    }
    if (subjEl && o.subject) {
      var sv = String(o.subject).trim();
      var opts = subjEl.options;
      var found = false;
      for (var i = 0; i < opts.length; i++) {
        if (opts[i].value === sv || String(opts[i].textContent || "").trim() === sv) {
      ...
      if (!found) {
        for (var j = 0; j < opts.length; j++) {
          if (opts[j].value === "Diğer") {
            subjEl.selectedIndex = j;
            break;
          }
        }
      }
    }
    if (typeof tmSyncPaperHeaders === "function") tmSyncPaperHeaders();
    if (Array.isArray(o.allTopics) && o.allTopics.length > 1) {
      try {
        sessionStorage.setItem("gorusmeOdasiAiPrefillQueue", JSON.stringify(o.allTopics.slice(1)));
      } catch (e2) {}
      showToast(
        "Kanayan " +
          o.allTopics.length +
          " konu için sırayla: önce «" +
          (o.display || o.topic || "") +
          "» alanlara yazıldı; diğerleri kuyrukta."
      );
    } else {
      showToast("Görüşme odasından ders/konu Test Oluşturucu’ya aktarıldı.");
    }
  } catch (e) {
    console.warn("[gorusme-odasi] prefill:", e);
  }
}
...
/** global_denemeler satırı için görünen tür: TYT / AYT / YKS (filtre mantığıyla uyumlu). */
function deriveGlobalExamKindForRow(r) {
  var t = String(r.sinavTuru || "").toUpperCase().trim();
  var ad = String(r.adi || "").toUpperCase();
  if (t === "TYT") return "TYT";
  if (t === "AYT") return "AYT";
  var hasT = ad.indexOf("TYT") !== -1;
  var hasA = ad.indexOf("AYT") !== -1;
  if (hasT && hasA) return "YKS";
  if (hasA) return "AYT";
  if (hasT) return "TYT";
  return "YKS";
}

function globalExamTypePillHtml(kind) {
  var cls =
    kind === "TYT"
...
function renderGlobalDenemeTakvimDataTable(rows) {
  ...
  if (!sorted.length) {
    tbody.innerHTML =
      ...
      '<span class="text-4xl text-gray-200" aria-hidden="true"><i class="fa-regular fa-file-lines"></i></span>' +
      '<p class="m-0 text-sm font-medium text-gray-400">Merkezi takvimde kayıt yok</p>' +
      "</div></td></tr>";
    return;
  }
  tbody.innerHTML = sorted
    .map(function (r) {
      ...
      var kind = deriveGlobalExamKindForRow(r);
      var typePill = globalExamTypePillHtml(kind);
      return (
        '<tr class="border-b border-slate-100/90 hover:bg-slate-50/80 transition-colors">' +
        '<td class="px-5 py-4">' +
        '<div class="flex flex-col gap-1.5 min-[480px]:flex-row min-[480px]:flex-wrap min-[480px]:items-start min-[480px]:gap-2">' +
        '<span class="text-sm font-semibold text-slate-800 leading-snug">' +
        escapeHtml(r.adi || "—") +
        "</span>" +
        '<span class="shrink-0 min-[480px]:mt-0.5">' +
        typePill +
        "</span></div></td>" +
        ...
        dateCell +
        "</td>" +
        '<td class="px-5 py-4 text-sm text-slate-600 whitespace-nowrap align-middle">' +
        escapeHtml(kind) +
        "</td>" +
        '<td class="px-5 py-4">' +
        badge +
...
/** @type {object[]|null} */
var dpHsFlatRows = null;
var dpHsSyncing = false;

function dpHsDestroySelect2(el) {
  if (!el || typeof jQuery === "undefined" || !jQuery.fn.select2) return;
  var $e = jQuery(el);
  if ($e.length && $e.hasClass("select2-hidden-accessible")) $e.select2("destroy");
}

function dpHsRebindSelect2(uSel, dSel) {
  dpHsDestroySelect2(uSel);
  dpHsDestroySelect2(dSel);
...
function dpHsSyncCrossFilter(uSel, dSel) {
  if (!dpHsFlatRows || !uSel || !dSel) return;
  dpHsSyncing = true;
  try {
    var sel = {
      universite: String(uSel.value || "").trim(),
      bolum: String(dSel.value || "").trim(),
    };
    pruneUniBolumSelections(dpHsFlatRows, sel);
    var opts = buildUniBolumCrossOptionSets(dpHsFlatRows, sel);
    dpHsDestroySelect2(uSel);
    dpHsDestroySelect2(dSel);
    uSel.innerHTML = '<option value="">— Üniversite seçin —</option>';
    opts.unis.forEach(function (u) {
      var o = document.createElement("option");
      o.value = u;
      o.textContent = u;
      uSel.appendChild(o);
    ...
    opts.bolums.forEach(function (b) {
      var o2 = document.createElement("option");
      o2.value = b;
      o2.textContent = b;
      dSel.appendChild(o2);
    });
    uSel.value = sel.universite || "";
    dSel.value = sel.bolum || "";
    dpHsRebindSelect2(uSel, dSel);
  } finally {
    dpHsSyncing = false;
  }
}

function dpHsWireAtlasSelectsOnce() {
  var uSel = document.getElementById("dpHsUniSelect");
  var dSel = document.getElementById("dpHsDeptSelect");
  if (!uSel || !dSel || !dpHsFlatRows || uSel.dataset.dpHsWired) return;
  uSel.dataset.dpHsWired = "1";
  function schedule() {
    requestAnimationFrame(function () {
      if (dpHsSyncing) return;
      dpHsSyncCrossFilter(uSel, dSel);
      renderDpHedefSimulatorChartsOnly();
    });
  }
  var empty = { universite: "", bolum: "" };
  var opts = buildUniBolumCrossOptionSets(dpHsFlatRows, empty);
  uSel.innerHTML = '<option value="">— Üniversite seçin —</option>';
  opts.unis.forEach(function (u) {
  ...
  if (typeof jQuery !== "undefined" && jQuery.fn.select2) {
    jQuery(uSel).on("change.dpHs select2:select.dpHs select2:clear.dpHs", schedule);
    jQuery(dSel).on("change.dpHs select2:select.dpHs select2:clear.dpHs", schedule);
  } else {
    uSel.addEventListener("change", schedule);
    dSel.addEventListener("change", schedule);
  }
}

function dpHsAlanKeyForResolver(pdoc, prog) {
...
function dpHedefClearSimulatorVisuals() {
  ...
  var examWarn = document.getElementById("dpHedefExamWarn");
...
function renderDpHedefSimulatorChartsOnly() {
  ...
  var uniName = uSel ? String(uSel.value || "").trim() : "";
  var bolName = dSel ? String(dSel.value || "").trim() : "";
  ...
  if (!uniName || !bolName) {
    ...
    if (uniEl) uniEl.textContent = "Hedef program seçin";
    if (bolEl) bolEl.textContent = "Üstteki üniversite ve bölümü seçin; ardından öğrenci seçerek karşılaştırın.";
    if (examWarn) {
      examWarn.hidden = true;
      examWarn.innerHTML = "";
    }
  ...
  var match = findBestCatalogProgramForFlatSelection(uniName, bolName, universities, programs);
  if (!match) {
    dpHedefClearSimulatorVisuals();
    if (uniEl) uniEl.textContent = uniName;
    if (bolEl) bolEl.textContent = bolName;
    if (tableWrap) {
      tableWrap.innerHTML =
        '<p class="net-sihirbazi-placeholder net-sihirbazi-placeholder--warn">yks-data.json içinde bu üniversite/bölüm için eşleşen program bulunamadı.</p>';
    }
    if (examWarn) {
      examWarn.hidden = true;
      examWarn.innerHTML = "";
  ...
  var prog = buildProgramFromAppwriteV2(match.uniDoc, match.programDoc, { exactYokAtlasTargets: true });
  if (!prog || !prog.rows || !prog.rows.length) {
    dpHedefClearSimulatorVisuals();
    if (uniEl) uniEl.textContent = hedefUniDisplayName(match.uniDoc) || uniName;
    if (bolEl) bolEl.textContent = hedefProgramDisplayName(match.programDoc) || bolName;
    if (tableWrap) {
      tableWrap.innerHTML =
        '<p class="net-sihirbazi-placeholder net-sihirbazi-placeholder--warn">Bu program için YÖK hedef net alanları katalogda yok.</p>';
    }
    if (examWarn) {
      examWarn.hidden = true;
      examWarn.innerHTML = "";
  ...
  if (uniEl) uniEl.textContent = prog.university || hedefUniDisplayName(match.uniDoc) || uniName;
  if (bolEl) bolEl.textContent = prog.department || hedefProgramDisplayName(match.programDoc) || bolName;
  ...
  var examPromise = sid
    ? sdFetchStudentExamAnalyticsDocs(sid, getCoachId())
    : Promise.resolve([]);

  examPromise
    .then(function (exams) {
      exams = Array.isArray(exams) ? exams : [];
      var noExamData = !sid || exams.length === 0;
      if (examWarn) {
        if (sid && exams.length === 0) {
          examWarn.hidden = false;
          examWarn.innerHTML =
            '<div class="rounded-xl border border-amber-200/90 bg-amber-50 px-4 py-3 text-sm text-amber-950">' +
            "<strong>Karşılaştırma için deneme ekleyin.</strong> Seçili öğrencinin Appwrite <code class=\"text-xs\">exams</code> kayıtlarında YKS branş detayı (yksBranchDetail) yok veya hiç deneme yok." +
            "</div>";
        } else {
          examWarn.hidden = true;
          examWarn.innerHTML = "";
      ...
      var alanKeyStr = dpHsAlanKeyForResolver(match.programDoc, prog);
      var resolver = createCurrentNetForRowResolver(exams, alanKeyStr);
      var display = buildMotorDisplayRows(prog, {
        currentNetForRow: resolver,
        exactYokAtlasTargets: true,
      });
      var forFilter = display.map(function (d) {
        return {
          label: d.label,
          current: d.current,
          target: d.target,
      ...
      var filtered = filterSimulatorRowsForStudentAlan(forFilter, alanKey);
      var keep = Object.create(null);
      filtered.forEach(function (x) {
        keep[x.label] = true;
      });
      var displayFiltered = display.filter(function (d) {
        return keep[d.label];
      });

      if (tableWrap) {
        tableWrap.innerHTML = hedefSimulatorComparisonHtml(displayFiltered, prog, {
          noExamData: noExamData,
          tableFootnote:
            "Fark = Güncel − Hedef. İlerleme çubuğu: güncel / hedef oranı (maks. %100). Öğrenci veya deneme yoksa güncel «Veri Yok».",
        });
      }

      if (barsEl) barsEl.innerHTML = "";

      var chartRows = displayFiltered.map(function (d) {
        return { label: d.label, current: d.current, target: d.target, section: d.section };
      });
      var totalGap = sumGap(chartRows);
      if (gapEl) {
        gapEl.textContent =
          "Branş toplamı kalan (hedef − güncel, pozitifler): " + totalGap.toFixed(1) + " net.";
      }
      ...
      if (probBlock && probPct && probFill) {
        if (chartRows.length) {
          ...
          var pctP = computeHedefWinProbabilityPercent(chartRows, prog.baseScore2025);
        ...
        var need = chartRows
          ...
          .map(function (r) {
            ...
            return (
              "<li><strong>" +
              escapeHtml(r.label) +
              "</strong> için yaklaşık <strong>" +
              g.toFixed(1) +
              "</strong> net daha gerekli.</li>"
            );
          })
          .join("");
        subGaps.innerHTML = need
          ? '<p class="dp-hedef-subgap__title">Öncelikli açıklar</p><ul class="dp-hedef-subgap__list">' +
            need +
            "</ul>"
          : '<p class="dp-hedef-subgap__muted">Tüm branşlarda hedefe yakınsınız veya veri eksik.</p>';
      }

      var labels = chartRows.map(function (r) {
        return r.label;
      });
      var current = chartRows.map(function (r) {
        return r.current;
      });
      var target = chartRows.map(function (r) {
        return r.target;
      });
    ...
    .catch(function (err) {
      console.error("[Hedef Simülatörü]", err);
    });
}
...
function renderDpHedefSimulator() {
  ...
  ensureHedefSimulatorAppwriteData()
    .then(function () {
      return fetchYokAtlasLisansFlatRows();
    })
    .then(function (rows) {
      dpHsFlatRows = rows || [];
      ...
      dpHsWireAtlasSelectsOnce();
      renderDpHedefSimulatorChartsOnly();
    })
    .catch(function (e) {
      console.error("[Hedef Simülatörü] yükleme:", e);
    });
}
...
/** Appwrite attribute sınırı için: tarih/saat/konu vb. details_json içinde saklanır */
function expandAppointmentData(ap) {
  var o = Object.assign({}, ap);
  if (typeof o.details_json === "string" && o.details_json) {
    try {
      var j = JSON.parse(o.details_json);
      if (j.studentName != null) o.studentName = j.studentName;
      if (j.studentId != null) o.studentId = j.studentId;
      if (j.date != null) o.date = j.date;
      if (j.time != null) o.time = j.time;
      if (j.durationMin != null) o.durationMin = j.durationMin;
      if (j.meetingType != null) o.meetingType = j.meetingType;
      if (j.topic != null) o.topic = j.topic;
      if (j.internalNotes != null) o.internalNotes = j.internalNotes;
      if (j.locationOrLink != null) o.locationOrLink = j.locationOrLink;
    } catch (_e) {}
  }
  return o;
}
...
function buildAppointmentList(docs) {
  return docs
    .map(function (docSnap) {
      return expandAppointmentData(Object.assign({}, docSnap.data(), { id: docSnap.id }));
    })
    .sort(function (a, b) {
...
/** Denemeler tam sayfa görünümü — `examsPageFilter` + arama */
function examMatchesPageViewFilters(row) {
  const tur = (row.examType || row.type || row.tur || "TYT").toUpperCase();
  if (examsPageFilter === "TYT" && tur !== "TYT") return false;
  if (examsPageFilter === "AYT" && tur !== "AYT") return false;
  const q = searchQuery.trim().toLowerCase();
  if (q) {
    const blob =
      (row.studentName || row.ogrenciAdi || row.name || "") +
...
function renderDashboardExams(flashExamId) {
  ...
  if (flashExamId) {
    var fid = escapeHtml(String(flashExamId));
    var tr = tbody.querySelector('tr[data-exam-id="' + fid + '"]');
    if (tr) {
      tr.classList.add("yks-row-enter");
      setTimeout(function () {
        tr.classList.remove("yks-row-enter");
      }, 600);
    }
  }
}
...
async function migrateStudentDocumentId(oldId, newIdRaw) {
  ...
  delete pdata.id;
  try {
    var wrSet = await setDoc(newRef, pdata);
    if (isAppwriteWriteSoftFailure(wrSet)) {
      showToast("Yeni öğrenci kaydı oluşturulamadı. Koleksiyon veya Appwrite izinlerini kontrol edin.");
      return;
    }
    try {
      var qAsn = query(
        collection(db, APPWRITE_COLLECTION_ATANAN_KAYNAKLAR),
        where("student_id", "==", oldId)
      );
      var subSnap = await getDocs(qAsn);
      for (var si = 0; si < subSnap.docs.length; si++) {
        var uAsn = await updateDoc(doc(db, APPWRITE_COLLECTION_ATANAN_KAYNAKLAR, subSnap.docs[si].id), {
          student_id: newId,
        });
        if (isAppwriteWriteSoftFailure(uAsn)) {
          console.warn("[migrateStudentDocumentId] atanan_kaynaklar yumuşak iptal", subSnap.docs[si].id);
        }
      }
    } catch (asnErr) {
      console.warn("[migrateStudentDocumentId] atanan_kaynaklar:", asnErr);
    }
    async function patchCol(rows, colName) {
      for (var ri = 0; ri < rows.length; ri++) {
        var row = rows[ri];
        try {
          var uPatch = await updateDoc(doc(db, colName, row.id), { studentId: newId });
          if (isAppwriteWriteSoftFailure(uPatch)) {
            console.warn("[migrateStudentDocumentId] yumuşak iptal", colName, row.id, uPatch.message || "");
          }
        } catch (e) {
    ...
    for (si = 0; si < subSnap.docs.length; si++) {
      var dSub = await deleteDoc(doc(db, "students", oldId, "atananKaynaklar", subSnap.docs[si].id));
      if (isAppwriteWriteSoftFailure(dSub)) {
        console.warn("[migrateStudentDocumentId] alt kaynak silinemedi (yumuşak)", subSnap.docs[si].id);
      }
    }
    var delOld = await deleteDoc(oldRef);
    if (isAppwriteWriteSoftFailure(delOld)) {
      showToast("Eski kayıt silinemedi. Yeni ID ile kayıt oluşmuş olabilir; konsolu kontrol edin.");
      return;
    }
    try {
      var m = sdLoadNotesMap();
...
function sdEliteExamDateStr(e) {
  var d = e.examDate || e.date || e.createdAt || "";
  if (d && typeof d.toDate === "function") {
    try {
      return d.toDate().toISOString().slice(0, 10);
    } catch (err) {}
  }
  if (d && typeof d === "object" && d.seconds != null) {
    try {
...
function sdEliteNormalizeExamDoc(raw, source) {
  ...
  var netRaw = parseTrNum(raw.net);
  return {
    id: raw.id,
    source: source,
    examName: raw.examName || raw.exam || "Deneme",
    date: sdEliteExamDateStr(raw),
    examType: String(raw.examType || raw.type || raw.tur || "TYT").toUpperCase(),
    net: isNaN(netRaw) ? null : netRaw,
    yksBranchDetail: yks,
    coach_id: raw.coach_id,
  };
}

function sdEliteNormalizeExamResultDoc(raw) {
  var detail = {};
  try {
  ...
  var net = NaN;
  if (detail.computed && typeof detail.computed.totalNet === "number") net = detail.computed.totalNet;
  else {
    var sum = 0;
    var any = false;
    if (detail.rows && typeof detail.rows === "object") {
      Object.keys(detail.rows).forEach(function (k) {
        var n = karneNetFromRowEntry(detail.rows[k]);
        if (!isNaN(n)) {
          sum += n;
          any = true;
        }
  ...
  var yks = null;
  if (detail.rows && typeof detail.rows === "object" && Object.keys(detail.rows).length) yks = detail;
  else if (detail.bulkImport && detail.branchNets && typeof detail.branchNets === "object") yks = detail;
...
async function sdFetchStudentExamAnalyticsDocs(studentId, coachId) {
  var list = [];
  var sid = String(studentId || "").trim();
  if (!sid) return list;

  var q1 = query(collection(db, "exams"), where("studentId", "==", sid));
  var snap1 = await getDocs(q1);
  snap1.forEach(function (d) {
    var x = typeof d.data === "function" ? d.data() : {};
    if (coachId && String(x.coach_id || "") !== String(coachId)) return;
    list.push(sdEliteNormalizeExamDoc(Object.assign({ id: d.id }, x), "exams"));
  });

  var q2 = query(collection(db, APPWRITE_COLLECTION_EXAM_RESULTS), where("student_id", "==", sid));
  var snap2 = await getDocs(q2);
  snap2.forEach(function (d) {
    var x = typeof d.data === "function" ? d.data() : {};
    if (coachId && String(x.coach_id || "") !== String(coachId)) return;
    list.push(sdEliteNormalizeExamResultDoc(Object.assign({ id: d.id }, x)));
  });

  return list;
}

function sdEliteSortDocsChronological(docs) {
...
function sdEliteBranchSuccessPercents(detail, studentAlan) {
  var out = [];
  if (!detail || !detail.rows || typeof detail.rows !== "object") return out;
  var examMode = String(detail.examMode || "TYT").toUpperCase();
  var alan = String(detail.aytAlan || studentAlan || "sayisal");
  var tytMap = karneBuildTytBranchLabelMap();
  var aytMap = karneBuildAytBranchLabelMap(alan);
  var allowAyt = examMode === "AYT" ? karneAllowedAytKeysForAlan(alan) : null;
  Object.keys(detail.rows).forEach(function (k) {
    if (examMode === "AYT" && allowAyt) {
      if (!allowAyt[k] && !allowAyt[String(k).replace(/^ayt_/, "")]) return;
    }
    var row = detail.rows[k];
    if (!row || row.soru == null) return;
    var cl = clampDy(row.soru, row.d, row.y);
    var soru = Number(row.soru);
    if (isNaN(soru) || soru <= 0) return;
    var pct = Math.min(100, Math.max(0, (cl.d / soru) * 100));
    var label = examMode === "AYT" ? aytMap[k] || String(k).replace(/^ayt_/, "") : tytMap[k] || k;
    out.push({ label: label, pct: pct });
  });
  out.sort(function (a, b) {
    return b.pct - a.pct;
  });
...
function sdEliteComputeDeltaNet(docs) {
  var withNet = docs.filter(function (d) {
    return d.net != null && !isNaN(d.net);
  });
  if (withNet.length < 2) return { delta: null };
  var tyt = withNet.filter(function (d) {
    return d.examType === "TYT";
  });
  var series = tyt.length >= 2 ? tyt : withNet;
  if (series.length < 2) return { delta: null };
  var last = series[series.length - 1].net;
  var prev = series[series.length - 2].net;
  return { delta: last - prev };
}

function sdEliteTargetGapText(student, avgTyt) {
  var t = student && student.targetTytNet != null ? parseTrNum(student.targetTytNet) : NaN;
  if (isNaN(t)) return "Hedef tanımlı değil";
  if (isNaN(avgTyt)) return "Ortalama yok";
  var diff = t - avgTyt;
  if (Math.abs(diff) < 0.05) return "Hedefe ulaşıldı";
  if (diff > 0) return diff.toFixed(1) + " net kaldı";
  return Math.abs(diff).toFixed(1) + " net üstündesin";
}

function sdEliteUpdateKpis(docs, student) {
  var kTyt = document.getElementById("sdEliteKpiTytAvg");
  var kDelta = document.getElementById("sdEliteKpiDelta");
  var kGap = document.getElementById("sdEliteKpiTargetGap");
  var tytNets = docs
    .filter(function (d) {
      return d.examType === "TYT" && d.net != null && !isNaN(d.net);
    })
    .map(function (d) {
      return d.net;
    });
  var avgTyt = tytNets.length ? tytNets.reduce(function (a, b) {
      return a + b;
    }, 0) / tytNets.length : NaN;
  if (kTyt) kTyt.textContent = tytNets.length ? avgTyt.toFixed(2) : "—";

  var dInfo = sdEliteComputeDeltaNet(docs);
  if (kDelta) {
    var sp = kDelta.querySelector(".sd-elite-delta__text");
    kDelta.classList.remove("sd-elite-delta--up", "sd-elite-delta--down", "sd-elite-delta--flat");
    if (dInfo.delta == null || isNaN(dInfo.delta)) {
      kDelta.classList.add("sd-elite-delta--flat");
      if (sp) sp.textContent = "—";
    } else {
      var del = dInfo.delta;
      kDelta.classList.add(del > 0 ? "sd-elite-delta--up" : del < 0 ? "sd-elite-delta--down" : "sd-elite-delta--flat");
      var sign = del > 0 ? "+" : "";
      var arrow =
        del > 0
          ...
          : del < 0
            ...
            : "";
      if (sp) sp.innerHTML = arrow + sign + del.toFixed(1) + " net";
    }
  }

  if (kGap) kGap.textContent = sdEliteTargetGapText(student, avgTyt);
}

function sdEliteDestroyCharts() {
  var canvasT = document.getElementById("studentDetailTrendChart");
  var canvasS = document.getElementById("studentDetailSubjectChart");
  if (typeof Chart === "undefined") return;
  if (canvasT) {
    var c1 = Chart.getChart(canvasT);
    if (c1) c1.destroy();
  }
  if (canvasS) {
    var c2 = Chart.getChart(canvasS);
    if (c2) c2.destroy();
  }
}

function sdEliteRenderTrendChart(canvas, docs) {
  if (!canvas || typeof Chart === "undefined") return;
  var existing = Chart.getChart(canvas);
  if (existing) existing.destroy();

  var labels = docs.map(function (e, i) {
    var n = e.examName || "D" + (i + 1);
    var ds = e.date ? String(e.date).slice(5) : "";
    var lab = (ds ? ds + " · " : "") + n;
    return lab.length > 24 ? lab.slice(0, 22) + "…" : lab;
  });
  var tytData = docs.map(function (e) {
    if (e.examType !== "TYT") return null;
    return e.net != null && !isNaN(e.net) ? e.net : null;
  });
  var aytData = docs.map(function (e) {
    if (e.examType !== "AYT") return null;
    return e.net != null && !isNaN(e.net) ? e.net : null;
  });
    ...
    options: {
      ...
      scales: {
        x: {
          ...
          ticks: { color: "#64748b", maxRotation: 42 },
        },
        y: {
          beginAtZero: true,
          grid: { color: "rgba(148, 163, 184, 0.2)" },
...
function sdEliteRenderSubjectChart(canvas, docs, studentAlan) {
  if (!canvas || typeof Chart === "undefined") return;
  var existing = Chart.getChart(canvas);
  if (existing) existing.destroy();
  ...
  for (var i = docs.length - 1; i >= 0; i--) {
    var det = docs[i].yksBranchDetail;
    if (det && det.rows && typeof det.rows === "object" && Object.keys(det.rows).length) {
      pickDetail = det;
      break;
    }
  }

  var branches = pickDetail ? sdEliteBranchSuccessPercents(pickDetail, studentAlan) : [];
  var labels = branches.length ? branches.map(function (b) {
      return b.label;
    }) : ["Branş verisi yok"];
  var data = branches.length
    ? branches.map(function (b) {
        return Math.round(b.pct * 10) / 10;
      })
    : [0];
      ...
      datasets: [
        {
          label: "Başarı %",
          data: data,
          backgroundColor: branches.length
            ? labels.map(function (_, idx) {
                var a = 0.52 + (idx % 5) * 0.09;
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label: function (ctx) {
              return ctx.parsed && ctx.parsed.x != null ? ctx.parsed.x + " %" : "";
            },
          },
        },
      },
...
async function renderStudentDetailEliteAnalytics(sid) {
  var seq = ++sdEliteAnalyticsSeq;
  var errEl = document.getElementById("sdEliteAnalyticsErr");
  var emptyEl = document.getElementById("sdEliteAnalyticsEmpty");
  var wrapEl = document.getElementById("sdEliteChartsWrap");
  var canvasT = document.getElementById("studentDetailTrendChart");
  var canvasS = document.getElementById("studentDetailSubjectChart");
  ...
  var st = sid ? cachedStudents.find(function (x) { return x.id === sid; }) : null;
  var studentAlan = st ? normalizeStudentYksAlanKey(st) : "sayisal";

  if (!sid) {
    sdEliteDestroyCharts();
    if (emptyEl) emptyEl.hidden = true;
    if (wrapEl) wrapEl.hidden = false;
    sdEliteUpdateKpis([], st || {});
    return;
  }
  ...
  var docs = [];
  try {
    docs = await sdFetchStudentExamAnalyticsDocs(sid, getCoachId());
  } catch (e) {
    console.error("[EliteAnalytics]", e);
    if (seq !== sdEliteAnalyticsSeq) return;
    sdEliteDestroyCharts();
    if (errEl) {
      errEl.hidden = false;
      errEl.textContent =
        "Deneme verileri yüklenemedi. Bağlantıyı veya koleksiyon izinlerini kontrol edin.";
    }
    if (emptyEl) emptyEl.hidden = true;
    if (wrapEl) wrapEl.hidden = false;
    sdEliteUpdateKpis([], st || {});
  ...
  docs = sdEliteSortDocsChronological(docs);
  ...
  sdEliteUpdateKpis(docs, st || {});
  sdEliteRenderTrendChart(canvasT, docs);
  sdEliteRenderSubjectChart(canvasS, docs, studentAlan);
}
...
function renderStudentDetailPage() {
  ...
  void renderStudentDetailEliteAnalytics(sid);
  ...
  document.getElementById("btnOptikManualSave") &&
    document.getElementById("btnOptikManualSave").addEventListener("click", async function () {
      ...
      try {
        ...
        var _wrOptikExam = await addDoc(collection(db, "exams"), payload);
        if (notifyIfWriteSoftFail(_wrOptikExam)) return;
        showToast("Deneme Appwrite veritabanına kaydedildi.", { variant: "success" });
        var out = document.getElementById("optikManualResult");
    ...
    btnO.addEventListener("click", async function () {
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        try {
          ...
          var _wrBulkEx = await addDoc(collection(db, "exams"), row);
          if (notifyIfWriteSoftFail(_wrBulkEx)) {
            fail.push((stud.name || key) + ": izin veya şema hatası");
            continue;
          }
          ok++;
        } catch (err) {
...
function normalizeAppointmentDoc(d) {
  if (!d) return null;
  var raw = typeof d.data === "function" ? d.data() : d;
  return expandAppointmentData(raw);
}

/**
 * @param {{ animateKpi?: boolean, skipLoading?: boolean } | undefined} opts
 */
async function refreshDashboardAnalytics(opts) {
  var skipLoad = !!(opts && opts.skipLoading);
  var animateKpi = !!(opts && opts.animateKpi);
  if (!skipLoad) setDashboardKpisLoading(true);
  try {
    renderDashboardKpis(
      {
        ...
        appointmentSortTime: appointmentSortTime,
        countWeekAppointments: countAppointmentsThisWeek,
      },
      { animate: animateKpi }
    );
    renderInstitutionNetTrendChart(function () {
      return cachedExams;
    });
    renderDashboardAppointmentIntensityBar(function () {
      return cachedAppointments;
    });
    renderNetBasariChart(function () {
      return cachedStudents;
    });
    renderDashboardInsightTable();
    await refreshDashboardMeetingActivity();
  } catch (err) {
    console.warn("[dashboard analytics]", err);
    var insight = document.getElementById("dashboardInsightText");
    if (insight) {
      insight.innerHTML =
        "Özet yüklenirken sorun oluştu. Ağ veya Appwrite izinlerini kontrol edin; veriler geldikçe güncellenecektir.";
    }
  } finally {
    if (!skipLoad) setDashboardKpisLoading(false);
  }
}

function studentsAsSnapDocs(arr) {
  return (arr || []).map(function (s) {
    return { id: s.id, data: function () { return s; } };
  });
}

function renderStudentsList(docs, flashStudentId) {
  ...
  list.innerHTML = top
    .map(function (docSnap) {
      ...
      var sidAttr = escapeHtml(String((s && (s.$id != null ? s.$id : s.id)) || docSnap.id || ""));
    ...
    .join("");
  if (flashStudentId) {
    var fid = escapeHtml(String(flashStudentId));
    var li = list.querySelector('li[data-student-id="' + fid + '"]');
    if (li) {
      li.classList.add("yks-row-enter");
      setTimeout(function () {
        li.classList.remove("yks-row-enter");
      }, 600);
    }
  }
}
...
function refreshCoachAvatarPackCountLabel() {
  var el = document.getElementById("coachAvatarPackCount");
  if (!el) return;
  el.innerHTML =
    'Yüklü avatar sayısı: <strong>40</strong> <span style="opacity:.85">(20 Erkek, 20 Kadın)</span>';
  try {
    storage
      .listFiles({ bucketId: APPWRITE_BUCKET_AVATARLAR, queries: [] })
      .then(function (res) {
        var files = (res && res.files) || [];
        var n = files.filter(function (f) {
          var name = String((f && f.name) || "");
          return /^male-\d{2}\.png$/i.test(name) || /^female-\d{2}\.png$/i.test(name);
        }).length;
        if (n > 0) {
          el.innerHTML =
            'Yüklü avatar sayısı: <strong>' +
            n +
            '</strong> <span style="opacity:.85">(20 Erkek, 20 Kadın)</span>';
        }
      })
      .catch(function () {});
  } catch (_e) {}
...
async function fillProfileSettingsForm() {
  ...
  refreshCoachAvatarPackCountLabel();
...
async function submitProfileSettings() {
  ...
  if (modal) dpClearFieldErrors(modal);
  setProfileSettingsMsg("", false);
  ...
  if (!name) {
    ...
    dpSetFieldError("profileDisplayName", "Ad soyad boş olamaz.");
    dpFocusField("profileDisplayName");
    return;
  }
  var emailCh = email && email !== String(profileSettingsInitial.email || "").toLowerCase().trim();
  if (emailCh && eEl && !eEl.disabled) {
    var emOk = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
    if (!emOk) {
      setProfileSettingsTab(0);
      dpSetFieldError("profileEmail", "Geçerli bir e-posta adresi girin.");
      dpFocusField("profileEmail");
      return;
    }
  }
  ...
  if ((emailCh || passCh) && cur.length < 1) {
    ...
    dpSetFieldError("profileCurrentPassword", "E-posta veya şifre değişikliği için mevcut şifrenizi girin.");
    dpFocusField("profileCurrentPassword");
    return;
  }
  if (passCh) {
    if (nw.length < 8) {
      setProfileSettingsTab(1);
      dpSetFieldError("profileNewPassword", "Yeni şifre en az 8 karakter olmalıdır.");
      dpFocusField("profileNewPassword");
      return;
    }
    if (nw !== nw2) {
      setProfileSettingsTab(1);
      dpSetFieldError("profileNewPassword2", "Yeni şifreler eşleşmiyor.");
      dpFocusField("profileNewPassword2");
      return;
    }
  }
...
function onAppointmentsSnap(snap) {
  cachedAppointmentRawDocs = snap.docs.map(function (d) {
    return { id: d.id, data: function () { return d.data(); } };
  });
  cachedAppointments = buildAppointmentList(cachedAppointmentRawDocs);
  apptCarouselOffset = 0;
  renderDashboardAppointments();
  renderAppointmentsChart(cachedAppointmentRawDocs);
...
function onStudentsSnap(snap) {
  ...
  if (currentView === "kocluk-gorusmeleri") refreshKoclukGorusmeList();
  ...
  try {
    setExamStudentGuard(function (sid) {
      return (cachedStudents || []).some(function (x) {
...
function finalizeStudentRemovedFromPanel(docId) {
  ...
  renderStudentsPage();
  fillStudentSelects();
  renderStudentsList(studentsAsSnapDocs(cachedStudents));
...
async function firestoreDeleteConfirmed(collectionName, docId) {
  ...
  try {
    var _wrDelDoc = await deleteDoc(doc(db, collectionName, docId));
    if (notifyIfWriteSoftFail(_wrDelDoc)) return;
    ...
    console.error(err);
    showToast("Silinemedi: " + (err.message || err), { variant: "danger" });
  }
}

/** 40 HD avatar — Appwrite Storage `avatarlar` (20 Erkek + 20 Kadın); ayrıntı: js/yks-avatar-pack.js */
var erkekAvatarları = YKS_AVATAR_MALE_URLS;
var kadinAvatarları = YKS_AVATAR_FEMALE_URLS;
var studentAddAvatarState = {
  mode: "preset",
  url: erkekAvatarları[0],
  customDataUrl: "",
};
var studentEditAvatarState = {
  mode: "preset",
  url: erkekAvatarları[0],
  customDataUrl: "",
};
...
/**
 * Varsayılan avatar: yerel PNG yoksa Dicebear havuzu (getAvatarPoolByGender) ile aynı kaynak.
 */
function getAvatarByGender(cinsiyet) {
  var pool = getAvatarPoolByGender(cinsiyet);
...
function updateStudentAvatarMetaText(gender, isCustom) {
  ...
  meta.textContent = "YKS adayı avatar — 40 HD (20 Erkek, 20 Kadın), Appwrite Storage";
}
...
function updateEditStudentAvatarMeta(isCustom) {
  ...
  meta.textContent = isCustom
    ? "Kişisel resim"
    : "YKS adayı avatar — 40 HD (20 Erkek, 20 Kadın), Appwrite Storage";
}
...
function openAvatarGallerySheet(target) {
  ...
  var contextOk =
    (stModal && !stModal.hidden) || (profModal && !profModal.hidden);
  if (!contextOk) return;

  var pick =
    target === "edit" ? "edit" : target === "coach" ? "coach" : "add";
  window.__avatarPickTarget = pick;

  var sheet = document.getElementById("avatarGallerySheet");
  var titleEl = document.getElementById("avatarGalleryTitle");
  var grid = document.getElementById("avatarGalleryGrid");

  var pool;
  if (pick === "coach") {
    pool = YKS_AVATAR_ALL_40_URLS.slice();
    if (titleEl) titleEl.textContent = "HD Avatar seç (40 · 20 Erkek + 20 Kadın)";
  } else {
    var g = pick === "edit" ? getSelectedEditGender() : getSelectedAddGender();
    pool = g === "Kadın" ? kadinAvatarları.slice() : erkekAvatarları.slice();
    if (titleEl) {
      titleEl.textContent =
        g === "Kadın" ? "HD Avatar seç (20 Kadın)" : "HD Avatar seç (20 Erkek)";
    }
  }
  ...
  document.getElementById("btnDaSaveFirestore") &&
    document.getElementById("btnDaSaveFirestore").addEventListener("click", async function () {
      ...
      try {
        payload.createdAt = serverTimestamp();
        payload.coach_id = getCoachId();
        var _wrEdsExam = await addDoc(collection(db, "exams"), payload);
        if (notifyIfWriteSoftFail(_wrEdsExam)) return;
        showToast("Deneme Appwrite veritabanına kaydedildi.", { variant: "success" });
        renderExamsFullPage();
        renderDashboardExams();
      } catch (err) {
        console.error(err);
        showToast(err.message || String(err), { variant: "danger" });
      }
    });
...
/* --- Haftalık program — Appwrite coach_tasks (Görev Takibi ile tek kaynak) --- */
/** Merkezi müfredat: `yks-mufredat.js` → YKS2026_Mufredat / yks2026DersKeys */

var HP_GOREV_TIPI_LABELS = {
  konu_calisma: "📖 Konu çalışması",
  soru_cozum: "📝 Soru çözümü",
  brans_deneme: "🎯 Branş denemesi",
  genel_tekrar: "🔁 Genel tekrar",
  video: "▶ Video izleme",
};

function hpMapLegacyToolToGorevTipi(tool) {
  var t = String(tool || "konu").toLowerCase();
  if (t === "soru") return "soru_cozum";
  if (t === "deneme") return "brans_deneme";
  return "konu_calisma";
}

function hpMapGorevTipiToLegacy(gorevTipi) {
  var g = String(gorevTipi || "konu_calisma");
  if (g === "soru_cozum") return "soru";
  if (g === "brans_deneme") return "deneme";
  return "konu";
}

function hpGorevTipiFromMeta(meta) {
  if (meta && meta.gorevTipi) return meta.gorevTipi;
  if (meta && meta.taskType) return hpMapLegacyToolToGorevTipi(meta.taskType);
  return "konu_calisma";
}

function hpParseMetaFromDescription(description) {
  var s = String(description || "").trim();
  if (!s || s.charAt(0) !== "{") return { meta: null };
  try {
    var o = JSON.parse(s);
    if (o && o.v === 1) return { meta: o };
  } catch (e) {}
  return { meta: null };
}

function hpSerializeWeeklyMeta(meta) {
  return JSON.stringify(meta);
}

/**
 * Appwrite coach_tasks.description JSON — tek şema (v:1).
 * @param {object} o
 */
function hpBuildTaskMetaFromForm(o) {
  o = o || {};
  var n =
    o.targetQuestions != null && String(o.targetQuestions).trim() !== ""
      ? parseInt(String(o.targetQuestions).trim(), 10)
      : NaN;
  var em =
    o.estimatedMinutes != null && String(o.estimatedMinutes).trim() !== ""
      ? parseInt(String(o.estimatedMinutes).trim(), 10)
      : NaN;
  var gt = String(o.gorevTipi || "konu_calisma").trim();
  return {
    v: 1,
    gorevTipi: gt,
    taskType: hpMapGorevTipiToLegacy(gt),
    topic: String(o.topic || "").trim(),
    targetQuestions: !isNaN(n) && n > 0 ? n : null,
    resource: String(o.resource || "").trim(),
    videoUrl: String(o.videoUrl || "").trim(),
    notes: String(o.notes || "").trim(),
    estimatedMinutes: !isNaN(em) && em > 0 ? em : null,
  };
}

function hpBuildCoachTaskTitle(subject, meta) {
  var subj = String(subject || "").trim();
  var topic = meta && meta.topic ? String(meta.topic).trim() : "";
  var n = meta && meta.targetQuestions != null && !isNaN(Number(meta.targetQuestions)) && Number(meta.targetQuestions) > 0;
  var em = meta && meta.estimatedMinutes != null && !isNaN(Number(meta.estimatedMinutes)) && Number(meta.estimatedMinutes) > 0;
  var parts = [];
  if (subj) parts.push(subj);
  if (topic) parts.push(topic);
  if (n) parts.push(Number(meta.targetQuestions) + " Soru");
  if (em) parts.push("~" + Number(meta.estimatedMinutes) + " dk");
  return parts.length ? parts.join(" - ") : "Görev";
}

function hpCardSummaryLine(t) {
  var meta = t.hpMeta || hpParseMetaFromDescription(t.description).meta;
  return hpBuildCoachTaskTitle(t.subject, meta || { topic: "" });
}

function hpGetSubjectKeysFromMufredat() {
  var keys = yks2026DersKeys();
...
function hpTopicsForSubject(dersKey) {
  for (var ei = 0; ei < 3; ei++) {
    var ex = ["TYT", "AYT", "YDT"][ei];
    var arr = (YKS2026_Mufredat[ex] || {})[dersKey];
    if (Array.isArray(arr) && arr.length) return arr.slice();
  }
  return [];
}

function coachTaskDescriptionForKanban(t) {
  var meta = t.hpMeta || hpParseMetaFromDescription(t.description).meta;
  if (meta) {
    var lines = [];
    if (meta.gorevTipi && HP_GOREV_TIPI_LABELS[meta.gorevTipi]) {
      lines.push("Görev tipi: " + HP_GOREV_TIPI_LABELS[meta.gorevTipi]);
    }
    if (meta.resource) lines.push("Kaynak: " + meta.resource);
    if (meta.notes) lines.push(meta.notes);
    if (meta.videoUrl) lines.push("Video: " + meta.videoUrl);
    if (meta.targetQuestions != null && meta.targetQuestions > 0) lines.push("Hedef: " + meta.targetQuestions + " soru");
    if (meta.estimatedMinutes != null && meta.estimatedMinutes > 0) lines.push("Tahmini süre: " + meta.estimatedMinutes + " dk");
    if (lines.length) return lines.join("\n");
  }
  var raw = String(t.description || "").trim();
...
function buildOgrenciVerisiPayload(studentId) {
  ...
  for (i = 0; i < 7; i++) {
    ...
    var tasksForDay = (cachedCoachTasks || [])
      .filter(function (t) {
        return (
      ...
      .map(function (t) {
        var meta = t.hpMeta || hpParseMetaFromDescription(t.description).meta;
        var det = [];
        if (meta && meta.topic) det.push(String(meta.topic));
        if (meta && meta.resource) det.push("Kaynak: " + meta.resource);
        if (meta && meta.notes) det.push(meta.notes);
        return {
          id: t.id,
          title: hpCardSummaryLine(t),
          detail: det.join(" · ") || "—",
          done: false,
        };
      });
  ...
  var pool = (cachedCoachTasks || []).filter(function (t) {
  ...
  var nextMeta = next ? next.hpMeta || hpParseMetaFromDescription(next.description).meta : null;
  var nextTask = {
    title: next ? hpCardSummaryLine(next) : "—",
    detail: next
      ? (nextMeta && nextMeta.resource ? nextMeta.resource + " · " : "") +
        (nextMeta && nextMeta.topic ? nextMeta.topic : "") +
        " · Teslim: " +
        String(next.dueDate || "")
...
async function saveOgrenciVerisiBridge(opts) {
  ...
  try {
    ...
    if (prevExists) {
      ...
      if (pd && typeof pd.task_done_map_json === "string" && pd.task_done_map_json) {
        try {
          prevDone = JSON.parse(pd.task_done_map_json) || {};
        } catch (_e) {}
      } else if (pd && pd.taskDoneMap && typeof pd.taskDoneMap === "object") {
        prevDone = pd.taskDoneMap;
      }
    }
    ...
    await setDoc(planRef, {
      ...
      week_json: JSON.stringify(payload.week || []),
      nextTaskId: payload.nextTaskId,
      next_task_json: JSON.stringify(payload.nextTask || {}),
      gorev_snapshot_json: JSON.stringify(payload.gorevSnapshot || []),
      task_done_map_json: JSON.stringify(prevDone || {}),
      updatedAt: serverTimestamp(),
      coachId: cid || null,
    });
...
function normalizeGorevTask(t) {
  ...
  return {
    ...
    hpMeta: t.hpMeta != null ? t.hpMeta : null,
  };
}
...
function onCoachTasksSnap(snap) {
  ...
  cachedCoachTasks = snap.docs.map(function (d) {
    var data = d.data();
    var p = hpParseMetaFromDescription(data.description);
    return {
      id: d.id,
  ...
  try {
    ...
    for (var ki = 0; ki < 3; ki++) {
      ...
      for (var j = 0; j < arr.length; j++) {
        ...
        if (!t) continue;
        n++;
        var _wrMig = await addDoc(collection(db, "coach_tasks"), {
          coach_id: cid,
          title: t.title,
        ...
        if (notifyIfWriteSoftFail(_wrMig)) {
          console.warn("[görev migrate] Appwrite yazımı iptal edildi.");
          return;
        }
      }
    }
...
function removeGorevKanbanTask(taskId) {
  ...
  deleteDoc(doc(db, "coach_tasks", taskId))
    .then(function (r) {
      if (notifyIfWriteSoftFail(r)) return;
      showToast("Görev silindi.", { variant: "success" });
    })
    .catch(function (e) {
      console.error(e);
      showToast("Silinemedi.", { variant: "danger" });
    });
}
...
function addGorevKanbanTaskFromForm(payload) {
  ...
  addDoc(collection(db, "coach_tasks"), {
    ...
    .then(function (r) {
      if (notifyIfWriteSoftFail(r)) return;
      showToast("Ödev eklendi.", { variant: "success" });
    })
    .catch(function (e) {
      console.error(e);
      showToast("Kaydedilemedi.", { variant: "danger" });
    });
}
...
function editStudent(studentId) {
  ...
  updateStudentEditSubmitButtonMode(s);
}

/** Öğrenci düzenle modalında: tam portal yoksa «Giriş Bilgisi Oluştur», aksi halde «Güncelle». */
function updateStudentEditSubmitButtonMode(s) {
  var btn = document.getElementById("btnStudentEditSubmit");
  if (!btn || !s) return;
  function setCreate(on) {
    if (on) {
      btn.innerHTML = '<i class="fa-solid fa-user-plus"></i> Giriş Bilgisi Oluştur';
    } else {
...
async function submitStudentAddForm(e) {
  ...
  if (!first) {
    setStudentErpTab(0);
    dpSetFieldError("st_firstName", "Ad zorunludur.");
    dpFocusField("st_firstName");
    return;
  }
  if (!last) {
    setStudentErpTab(0);
    dpSetFieldError("st_lastName", "Soyad zorunludur.");
    dpFocusField("st_lastName");
    return;
  }
  data.name = (first + " " + last).trim();
  if (!data.parentPhone) {
    setStudentErpTab(2);
    dpSetFieldError("st_parentPhone", "Veli telefonu zorunludur.");
    dpFocusField("st_parentPhone");
    return;
  }
  data.phone = data.parentPhone;
  if (data.tcKimlikNo && String(data.tcKimlikNo).replace(/\D/g, "").length !== 11) {
    setStudentErpTab(0);
    dpSetFieldError("st_tcKimlikNo", "TCKN 11 hane olmalıdır (veya boş bırakın).");
    dpFocusField("st_tcKimlikNo");
    return;
  }
  var curNetChk = dpParseDecimalField(data.currentTytNet, { required: false, min: 0, max: 999 });
  if (!curNetChk.ok) {
    setStudentErpTab(1);
    dpSetFieldError("st_currentTytNet", curNetChk.message || "Geçersiz net.");
    dpFocusField("st_currentTytNet");
    return;
  }
  var tgtNetChk = dpParseDecimalField(data.targetTytNet, { required: false, min: 0, max: 999 });
  if (!tgtNetChk.ok) {
    setStudentErpTab(1);
    dpSetFieldError("st_targetTytNet", tgtNetChk.message || "Geçersiz net.");
    dpFocusField("st_targetTytNet");
    return;
  }
  ...
  if (wantAuth) {
    ...
    if (!u) {
      setStudentErpTab(1);
      dpSetFieldError("st_portalUsername", "Portal kullanıcı adı yalnızca a-z, 0-9 ve _ içerebilir.");
      dpFocusField("st_portalUsername");
      return;
    }
    if (pass.length < 8) {
      setStudentErpTab(1);
      dpSetFieldError("st_studentPassword", "Giriş şifresi en az 8 karakter olmalıdır.");
      dpFocusField("st_studentPassword");
      return;
    }
    if (pass !== pass2) {
      setStudentErpTab(1);
      dpSetFieldError("st_studentPasswordConfirm", "Şifreler eşleşmiyor.");
      dpFocusField("st_studentPasswordConfirm");
      return;
    }
  ...
  await runWithSubmitButtonBusy(btn, async function () {
    try {
      ...
      if (_wrAddStudent && _wrAddStudent.id) {
        ...
        if (cachedStudents.every(function (x) {
          return x.id !== sNew.id;
        })) {
          cachedStudents.unshift(sNew);
        }
        renderStudentsList(studentsAsSnapDocs(cachedStudents), _wrAddStudent.id);
        panelUpsertStudentCardInDom(sNew);
        fillStudentSelects();
        refreshPanelAfterRealtimeMutation();
      }
      dpToastApiSuccess();
      form.reset();
...
var bulkStuPendingFile = null;

function normalizeFieldTypeFromExcel(raw) {
  var s = String(raw || "")
    .trim()
    .toUpperCase()
    .replace(/İ/g, "I")
    .replace(/ı/g, "I");
  if (!s) return "";
  if (s.indexOf("SAY") !== -1 || s.indexOf("SAYISAL") !== -1) return "Sayısal";
  if (s.indexOf("EA") !== -1 || s.indexOf("EŞIT") !== -1 || s.indexOf("ESIT") !== -1 || s.indexOf("AĞIRLIK") !== -1)
    return "Eşit Ağırlık (EA)";
  if (s.indexOf("SÖZ") !== -1 || s.indexOf("SOZ") !== -1) return "Sözel";
  if (s.indexOf("DİL") !== -1 || s.indexOf("DIL") !== -1) return "Dil";
  return "";
}
...
function excelRowToStudentFieldMap(row) {
  ...
  return {
    ...
    firstName: firstName,
    lastName: lastName,
    studentPhone: pickBulkField(norm, [
      "studentPhone",
      "ogrenci_telefonu",
      "ogrenci_telefon",
      "ogrenci_gsm",
    ...
    targetDepartment: pickBulkField(norm, [
      "targetDepartment",
      "hedef_bolum",
      "hedef_bölüm",
      "bolum",
...
async function importSingleStudentFromExcelRow(fieldMap) {
  ...
  var studentPhone = (fieldMap.studentPhone || "").trim();
  if (!parentPhone) {
  ...
  data.phone = studentPhone || parentPhone;
  ...
  if (fieldMap.fieldType) {
    var normFt = normalizeFieldTypeFromExcel(fieldMap.fieldType);
    data.fieldType = normFt || fieldMap.fieldType;
  }
...
function isBulkStudentTemplateSampleRow(fm) {
  var a = String(fm.firstName || "")
    .trim()
    .toLowerCase()
    .replace(/i̇/g, "i");
  var b = String(fm.lastName || "")
    .trim()
    .toLowerCase()
    .replace(/i̇/g, "i");
  if ((a === "ayşe" || a === "ayse") && (b === "yılmaz" || b === "yilmaz")) return true;
  return false;
}

function resetBulkStudentImportModal() {
  bulkStuPendingFile = null;
  var inp = document.getElementById("bulkImportFileInput");
  var fn = document.getElementById("bulkImportFileName");
  var up = document.getElementById("btnBulkImportUpload");
  var err = document.getElementById("bulkImportXimErr");
  ...
  setBulkImportUploadLoading(false);
}

function setBulkImportUploadLoading(on) {
  var up = document.getElementById("btnBulkImportUpload");
  if (!up) return;
  var idle = up.querySelector(".bulk-import-upload__idle");
  var busy = up.querySelector(".bulk-import-upload__busy");
  if (idle) idle.hidden = !!on;
  if (busy) busy.hidden = !on;
  up.disabled = on ? true : !bulkStuPendingFile;
}

function applyBulkStudentPendingFile(f) {
  bulkStuPendingFile = f || null;
  var fn = document.getElementById("bulkImportFileName");
  var up = document.getElementById("btnBulkImportUpload");
  var err = document.getElementById("bulkImportXimErr");
...
function downloadBulkStudentTemplateXlsx() {
  ...
  var headers = [
    "Ad Soyad",
    "Durum (12. Sınıf/Mezun)",
    "Alan (SAY/EA/SÖZ/DİL)",
    "Öğrenci Telefon",
    "Veli Telefon",
    "Hedef Bölüm",
  ];
  var example = ["Ayşe Yılmaz", "12. Sınıf", "SAY", "05551112222", "05559998877", "Tıp"];
  var ws = XLSX.utils.aoa_to_sheet([headers, example]);
  ws["!cols"] = [{ wch: 28 }, { wch: 26 }, { wch: 24 }, { wch: 18 }, { wch: 16 }, { wch: 36 }];
  var wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Ogrenciler");
  XLSX.writeFile(wb, "ogrenci_toplu_sablon.xlsx");
}

async function runBulkStudentImportFromFile(file) {
  if (!file) return;
  var coachId = getCoachId();
  var errBox = document.getElementById("bulkImportXimErr");
  if (errBox) {
    errBox.hidden = true;
    errBox.textContent = "";
  }
  ...
  try {
    ...
    if (errBox) {
      errBox.textContent = rmsg;
      errBox.hidden = false;
    }
    showToast("Dosya okunamadı: " + rmsg, { variant: "danger" });
    return;
  }
  if (!rows.length) {
    var emptyMsg = "Tabloda veri satırı yok (başlık dışında).";
    if (errBox) {
      errBox.textContent = emptyMsg;
      errBox.hidden = false;
    }
    showToast(emptyMsg, { variant: "danger" });
    return;
  }
  var ok = 0;
  var skipped = 0;
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (isBulkStudentTemplateSampleRow(fm)) {
      skipped++;
      continue;
    }
    try {
  ...
  if (ok > 0) {
    showToast("Başarıyla " + ok + " öğrenci eklendi" + (errors.length ? " · " + errors.length + " satır hata." : "."), {
      variant: "success",
    });
  ...
  if (errors.length) {
    ...
    showToast("Boş veya örnek satırlar atlandı; kayıt eklenmedi.", { variant: "danger" });
  }
}
...
function initBulkStudentExcelImport() {
  ...
  if (bulkIn) {
    bulkIn.addEventListener("change", function () {
      ...
      if (f) applyBulkStudentPendingFile(f);
    });
  }
  ...
  if (dz && bulkIn) {
    ...
    dz.addEventListener("drop", function (e) {
      ...
      if (fl && fl[0]) applyBulkStudentPendingFile(fl[0]);
    });
  }

  var btnUp = document.getElementById("btnBulkImportUpload");
  if (btnUp) {
    btnUp.addEventListener("click", function () {
      if (!bulkStuPendingFile) return;
      var errEl = document.getElementById("bulkImportXimErr");
      if (errEl) {
        errEl.hidden = true;
        errEl.textContent = "";
      }
      setBulkImportUploadLoading(true);
      void runBulkStudentImportFromFile(bulkStuPendingFile).finally(function () {
        setBulkImportUploadLoading(false);
        var up2 = document.getElementById("btnBulkImportUpload");
        if (up2 && bulkStuPendingFile) up2.disabled = false;
      });
    });
  }
...
function mergeStudentEditPortalDbFields(oldSt, data, gv) {
  ...
  if (newPass.length > 0 && newPass.length < 6) {
    dpSetFieldError("editStudentLoginPassword", "Şifre en az 6 karakter olmalıdır.");
    dpFocusField("editStudentLoginPassword");
    return { ok: false };
  }
  if (newPass.length > 0 && !u) {
    dpSetFieldError("editStudentLoginPassword", "Şifre atamak için geçerli bir kullanıcı adı girin.");
    dpFocusField("editStudentLoginUsername");
    return { ok: false };
  }
...
async function submitStudentEditForm(e) {
  ...
  if (!editId) {
    dpToastApiError("Geçersiz kayıt.");
    return;
  }
  ...
  if (!first) {
    dpSetFieldError("editStudentName", "Ad zorunludur.");
    dpFocusField("editStudentName");
    return;
  }
  if (!last) {
    dpSetFieldError("editStudentSurname", "Soyad zorunludur.");
    dpFocusField("editStudentSurname");
    return;
  }
  var parentPhone = (document.getElementById("editParentPhone") || {}).value;
  parentPhone = parentPhone ? String(parentPhone).trim() : "";
  if (!parentPhone) {
    dpSetFieldError("editParentPhone", "Veli telefonu zorunludur.");
    dpFocusField("editParentPhone");
    return;
  }
  ...
  if (tckn && String(tckn).replace(/\D/g, "").length !== 11) {
    dpSetFieldError("editTcKimlikNo", "TCKN 11 hane olmalıdır (veya boş bırakın).");
    dpFocusField("editTcKimlikNo");
    return;
  }
  var curNetEl = gv("editCurrentTytNet");
  var curNetChk = dpParseDecimalField(curNetEl, { required: false, min: 0, max: 999 });
  if (!curNetChk.ok) {
    dpSetFieldError("editCurrentTytNet", curNetChk.message || "Geçersiz net.");
    dpFocusField("editCurrentTytNet");
    return;
  }
  var tgtNetEl = gv("editTargetTytNet");
  var tgtNetChk = dpParseDecimalField(tgtNetEl, { required: false, min: 0, max: 999 });
  if (!tgtNetChk.ok) {
    dpSetFieldError("editTargetTytNet", tgtNetChk.message || "Geçersiz net.");
    dpFocusField("editTargetTytNet");
    return;
  }
  var data = {
    firstName: first,
    ...
    currentTytNet: curNetEl || null,
    targetTytNet: tgtNetEl || null,
  ...
  await runWithSubmitButtonBusy(btn, async function () {
    try {
      ...
      var mergedSt = Object.assign({}, ixEd >= 0 ? cachedStudents[ixEd] : {}, data, { id: editId });
      if (ixEd >= 0) Object.assign(cachedStudents[ixEd], mergedSt);
      else cachedStudents.push(mergedSt);
      panelUpsertStudentCardInDom(ixEd >= 0 ? cachedStudents[ixEd] : mergedSt);
      renderStudentsList(studentsAsSnapDocs(cachedStudents));
      fillStudentSelects();
      refreshPanelAfterRealtimeMutation();
      if (currentStudentDetailId === editId) refreshStudentDetailIfOpen();
      if (sync.portalInitMode) {
        dpToastApiSuccess("Öğrenci için giriş bilgileri başarıyla oluşturuldu ve tanımlandı.");
      } else if (sync.portalAuthChanged) {
        dpToastApiSuccess("Giriş bilgileri başarıyla güncellendi.");
      } else {
        dpToastApiSuccess();
      }
      document.getElementById("formStudentEdit").reset();
      document.getElementById("editStudentDocId").value = "";
      var elPwClear = document.getElementById("editStudentLoginPassword");
      if (elPwClear) elPwClear.value = "";
      resetStudentModalPanes();
      closeAllModals();
    } catch (err) {
      console.error(err);
...
async function submitAppointmentForm(e) {
  ...
  if (!st || !sid) {
    dpSetFieldError("ap_student", "Öğrenci seçin.");
    dpFocusField("ap_student");
    return;
  }
  var d = (fd.get("appointmentDate") && String(fd.get("appointmentDate")).trim()) || "";
  var tm = (fd.get("appointmentTime") && String(fd.get("appointmentTime")).trim()) || "";
  if (!d) {
    dpSetFieldError("ap_date", "Tarih seçin.");
    dpFocusField("ap_date");
    return;
  }
  if (!tm) {
    dpSetFieldError("ap_time", "Saat seçin.");
    dpFocusField("ap_time");
    return;
  }
  var sched = new Date(d + "T" + tm);
  if (isNaN(sched.getTime())) {
    dpSetFieldError("ap_time", "Geçerli bir tarih ve saat girin.");
    dpFocusField("ap_time");
    return;
  }
  var details = {
  ...
  if (editAppt) {
    details.updatedAtIso = new Date().toISOString();
  } else {
    details.createdAtIso = new Date().toISOString();
  }
  var detailsJson = JSON.stringify(details);
  if (detailsJson.length > 3000) {
    dpSetFieldError("ap_notes", "Randevu detayı çok uzun (en fazla 3000 karakter). Notları kısaltın.");
    dpFocusField("ap_notes");
    return;
  }
  var payload = {
  ...
  var btn = document.getElementById("btnApptSubmit");
  await runWithSubmitButtonBusy(btn, async function () {
    try {
      if (editAppt) {
        var _wrApU = await updateDoc(doc(db, "appointments", editAppt), payload);
        if (notifyIfWriteSoftFail(_wrApU)) return;
      } else {
        var _wrApA = await addDoc(collection(db, "appointments"), payload);
        if (notifyIfWriteSoftFail(_wrApA)) return;
      }
      dpToastApiSuccess();
...
async function onPdfTaslakClick() {
  ...
  var btn = document.getElementById("btnPdfTaslak");
  await runWithSubmitButtonBusy(btn, async function () {
    try {
      if (editTestId) {
        var _wrTmU = await updateDoc(doc(db, "tests", editTestId), {
          ...payload,
          module: "TestMakerPro",
          pdfDraft: true,
          updatedAt: serverTimestamp(),
        });
        if (notifyIfWriteSoftFail(_wrTmU)) return;
      } else {
        var _wrTmA = await addDoc(collection(db, "tests"), {
          ...payload,
          module: "TestMakerPro",
          pdfDraft: true,
          status: "Taslak",
        ...
        if (notifyIfWriteSoftFail(_wrTmA)) return;
      }
      dpToastApiSuccess();
      resetTestMakerModal();
      closeAllModals();
...
async function submitPaymentForm(e) {
  ...
  if (!st || !sid) {
    dpSetFieldError("pay_student", "Öğrenci seçin.");
    dpFocusField("pay_student");
    return;
  }
  var amountRaw = fd.get("amount");
  var amt = dpParseDecimalField(amountRaw, { required: true, min: 0.01, max: 9999999 });
  if (!amt.ok) {
    dpSetFieldError("pay_amount", amt.message || "Geçerli bir tutar girin.");
    dpFocusField("pay_amount");
    return;
  }
  var payload = {
    studentId: sid,
    studentName: st.name || st.studentName || "",
    amount: parseFloat(String(amountRaw).replace(",", "."), 10),
    paymentDate: fd.get("paymentDate") || new Date().toISOString().slice(0, 10),
    paymentMethod: fd.get("paymentMethod") || "",
    description: fd.get("description") || "",
    invoiceNote: fd.get("invoiceNote") || "",
  };
  var btn = document.getElementById("btnPaymentSubmit");
  await runWithSubmitButtonBusy(btn, async function () {
    try {
      if (editPay) {
        payload.updatedAt = serverTimestamp();
        var _wrPayU = await updateDoc(doc(db, "payments", editPay), payload);
        if (notifyIfWriteSoftFail(_wrPayU)) return;
      } else {
        payload.createdAt = serverTimestamp();
        payload.coach_id = getCoachId();
        var _wrPayA = await addDoc(collection(db, "payments"), payload);
        if (notifyIfWriteSoftFail(_wrPayA)) return;
      }
      dpToastApiSuccess();
      form.reset();
      var h = document.getElementById("paymentEditDocId");
...
async function submitExamForm(e) {
  ...
  if (!st || !sid) {
    dpSetFieldError("ex_student", "Öğrenci seçin.");
    dpFocusField("ex_student");
    return;
  }
  var netRaw = fd.get("net");
  var netChk = dpParseDecimalField(netRaw, { required: true, min: 0, max: 200 });
  if (!netChk.ok) {
    dpSetFieldError("ex_net", netChk.message || "Geçerli bir net girin.");
    dpFocusField("ex_net");
    return;
  }
  ...
  if (exD) {
    var d0 = new Date(exD);
    if (isNaN(d0.getTime())) {
      dpSetFieldError("ex_date", "Geçerli bir deneme tarihi seçin.");
      dpFocusField("ex_date");
      return;
    }
    examDateTs = Timestamp.fromDate(d0);
  }
  var payload = {
    studentId: sid,
    studentName: st.name || st.studentName || "",
  ...
  var btn = document.getElementById("btnExamSubmit");
  await runWithSubmitButtonBusy(btn, async function () {
    try {
      if (editEx) {
        payload.updatedAt = serverTimestamp();
        var _wrExU = await updateDoc(doc(db, "exams", editEx), payload);
        if (notifyIfWriteSoftFail(_wrExU)) return;
        var ixEu = cachedExams.findIndex(function (x) {
          return x.id === editEx;
        });
        var mergedEx = Object.assign({}, ixEu >= 0 ? cachedExams[ixEu] : {}, payload, { id: editEx });
        if (ixEu >= 0) Object.assign(cachedExams[ixEu], mergedEx);
        else cachedExams.push(mergedEx);
        panelReplaceExamRowsInDom(ixEu >= 0 ? cachedExams[ixEu] : mergedEx);
        refreshPanelAfterRealtimeMutation();
        refreshStudentDetailIfOpen();
        if (currentView === "karne") renderKarneReport();
        dispatchGoCockpitRefresh();
        ...
        payload.createdAt = serverTimestamp();
        payload.coach_id = getCoachId();
        var _wrExA = await addDoc(collection(db, "exams"), payload);
        if (notifyIfWriteSoftFail(_wrExA)) return;
        if (_wrExA && _wrExA.id) {
          var optEx = Object.assign({}, payload, { id: _wrExA.id });
          if (cachedExams.every(function (x) {
            return x.id !== optEx.id;
          })) {
            cachedExams.push(optEx);
          }
          panelInsertExamRowDomIncremental(optEx);
          refreshPanelAfterRealtimeMutation();
          refreshStudentDetailIfOpen();
          if (currentView === "karne") renderKarneReport();
          dispatchGoCockpitRefresh();
...
function coachDocBelongsToRealtimeDoc(x) {
...
function panelRealtimeDocId(payload) {
  if (!payload || typeof payload !== "object") return "";
  return String(payload.$id != null ? payload.$id : payload.id || "").trim();
}
...
function trimExamTbodyToMaxRows(tbody, maxRows) {
  ...
  var n;
  do {
    n = tbody.querySelectorAll("tr[data-exam-id]").length;
    if (n <= maxRows) break;
    var last = tbody.querySelector("tr[data-exam-id]:last-of-type");
    if (last) last.remove();
    else break;
  } while (n > maxRows);
}
...
/**
 * Tek deneme satırını sıralı ekler / günceller (dashboard en fazla maxRows).
 */
function insertExamRowIntoTbody(tbody, row, maxRows) {
  if (!tbody || !row || !row.id) return;
  var eidEsc = escapeHtml(String(row.id));
  var existing = tbody.querySelector('tr[data-exam-id="' + eidEsc + '"]');
  if (existing) {
    panelReplaceExamRowsInDom(row);
    trimExamTbodyToMaxRows(tbody, maxRows);
    return;
  }
  var tpl = document.createElement("tbody");
  tpl.innerHTML = examRowHtml(row, 6).trim();
  var fresh = tpl.firstElementChild;
  if (!fresh) return;
  var ts = examDateSort(row);
  var rows = Array.prototype.slice.call(tbody.querySelectorAll("tr[data-exam-id]"));
  var insertBeforeRef = null;
  for (var i = 0; i < rows.length; i++) {
    var rts = parseInt(rows[i].getAttribute("data-exam-ts") || "0", 10) || 0;
    if (rts < ts) {
      insertBeforeRef = rows[i];
      break;
    }
  }
  var emptyCell = tbody.querySelector("td.table-empty");
  if (emptyCell && emptyCell.parentElement) {
    emptyCell.parentElement.remove();
  }
  fresh.classList.add("yks-row-enter");
  if (insertBeforeRef) tbody.insertBefore(fresh, insertBeforeRef);
  else tbody.appendChild(fresh);
  setTimeout(function () {
    fresh.classList.remove("yks-row-enter");
  }, 620);
  trimExamTbodyToMaxRows(tbody, maxRows);
}

/** Realtime / optimistic: tam sayfa innerHTML yerine satır ekle-güncelle */
function panelInsertExamRowDomIncremental(row) {
  var dash = document.getElementById("denemeTableBody");
  if (dash && examMatchesFilters(row)) {
    var plain = cachedExams.slice().sort(function (a, b) {
      return examDateSort(b) - examDateSort(a);
    });
    var filtered = plain.filter(examMatchesFilters);
    var ix = filtered.findIndex(function (x) {
      return x.id === row.id;
    });
    if (ix >= 0 && ix < 15) {
      insertExamRowIntoTbody(dash, row, 15);
    }
  }
  var full = document.getElementById("examsPageBody");
  if (full && examMatchesPageViewFilters(row)) {
    insertExamRowIntoTbody(full, row, 0);
  }
}

function panelReplaceExamRowsInDom(row) {
  var html = examRowHtml(row, 6);
  var tpl = document.createElement("tbody");
  tpl.innerHTML = html.trim();
  var fresh = tpl.firstElementChild;
  if (!fresh) return;
  ["denemeTableBody", "examsPageBody"].forEach(function (tid) {
    var tb = document.getElementById(tid);
    if (!tb) return;
    var old = tb.querySelector('tr[data-exam-id="' + escapeHtml(String(row.id)) + '"]');
    if (old) {
      old.replaceWith(/** @type {HTMLTableRowElement} */ (fresh.cloneNode(true)));
    }
  });
}

function panelFadeRemoveExamRows(examId, done) {
  var nodes = Array.prototype.slice.call(
    document.querySelectorAll('tr[data-exam-id="' + escapeHtml(String(examId)) + '"]')
  );
  if (!nodes.length) {
    if (done) done();
    return;
  ...
  nodes.forEach(function (tr) {
    tr.classList.add("yks-row-leave");
  });
  setTimeout(function () {
    nodes.forEach(function (tr) {
      try {
...
function panelFadeStudentElements(studentId, done) {
  var nodes = Array.prototype.slice.call(
    document.querySelectorAll('[data-student-id="' + escapeHtml(String(studentId)) + '"]')
  );
  if (!nodes.length) {
    if (done) done();
    return;
  ...
  nodes.forEach(function (el) {
    el.classList.add("yks-row-leave");
  });
  setTimeout(function () {
    nodes.forEach(function (el) {
      try {
...
function coachTaskRowFromRealtimePayload(payload, docId) {
  var data = panelMergePlainFromPayload(payload);
  var p = hpParseMetaFromDescription(data.description || "");
  return {
    id: docId,
    coach_id: data.coach_id,
...
function refreshPanelAfterRealtimeMutation() {
  var dash = currentView === "dashboard";
  void refreshDashboardAnalytics({ skipLoading: true, animateKpi: dash });
}

function applyPanelRealtimeDocEvent(detail) {
  if (!kocPanelBootstrapped || !getCoachIdResolved() || !detail || !detail.op || !detail.collectionId) return;
  var op = detail.op;
  var coll = detail.collectionId;
  var payload = detail.payload;
  var id = panelRealtimeDocId(payload);
  if (!id) return;

  if (coll === "exams") {
    if (op === "delete") {
      var ixDel = cachedExams.findIndex(function (x) {
        return x.id === id;
      });
      if (ixDel < 0) return;
      cachedExams.splice(ixDel, 1);
      panelFadeRemoveExamRows(id, function () {
        renderDashboardExams();
        renderExamsFullPage();
        refreshPanelAfterRealtimeMutation();
        refreshStudentDetailIfOpen();
    ...
    var erow = panelMergePlainFromPayload(payload);
    erow.id = id;
    if (!coachDocBelongsToRealtimeDoc(erow)) {
      var ixN = cachedExams.findIndex(function (x) {
        return x.id === id;
      });
      if (ixN >= 0) cachedExams.splice(ixN, 1);
      renderDashboardExams();
      renderExamsFullPage();
      refreshPanelAfterRealtimeMutation();
      return;
    }
    var ixE = cachedExams.findIndex(function (x) {
      return x.id === id;
    });
    if (ixE >= 0) Object.assign(cachedExams[ixE], erow);
    else cachedExams.push(erow);
    var mergedE = cachedExams.find(function (x) {
      return x.id === id;
    });
    if (op === "update" && mergedE) {
      panelReplaceExamRowsInDom(mergedE);
      refreshPanelAfterRealtimeMutation();
      refreshStudentDetailIfOpen();
      if (currentView === "karne") renderKarneReport();
      dispatchGoCockpitRefresh();
    ...
    if (mergedE) {
      if (op === "create") {
        panelInsertExamRowDomIncremental(mergedE);
        refreshPanelAfterRealtimeMutation();
        refreshStudentDetailIfOpen();
        if (currentView === "karne") renderKarneReport();
        dispatchGoCockpitRefresh();
  ...
  if (coll === "students") {
    if (op === "delete") {
      var sx = cachedStudents.findIndex(function (x) {
        return x.id === id;
      });
      if (sx < 0) return;
      cachedStudents.splice(sx, 1);
      panelFadeStudentElements(id, function () {
        renderStudentsList(studentsAsSnapDocs(cachedStudents));
        renderStudentsPage();
        fillStudentSelects();
        refreshPanelAfterRealtimeMutation();
      ...
      if (currentView === "ogrenci-detay" && currentStudentDetailId === id) {
        currentStudentDetailId = null;
        showToast("Öğrenci silindi veya erişilemiyor.");
        navigateTo("ogrenciler");
      }
    ...
    var srow = panelMergePlainFromPayload(payload);
    srow.id = id;
    if (!coachDocBelongsToRealtimeDoc(srow)) {
      var ixSt = cachedStudents.findIndex(function (x) {
        return x.id === id;
      });
      if (ixSt >= 0) {
        cachedStudents.splice(ixSt, 1);
        renderStudentsList(studentsAsSnapDocs(cachedStudents));
        renderStudentsPage();
        fillStudentSelects();
      }
    ...
    var ixS = cachedStudents.findIndex(function (x) {
      return x.id === id;
    });
    if (ixS >= 0) Object.assign(cachedStudents[ixS], srow);
    else cachedStudents.push(srow);
    renderStudentsList(studentsAsSnapDocs(cachedStudents), op === "create" ? id : undefined);
    panelUpsertStudentCardInDom(srow);
    fillStudentSelects();
    refreshPanelAfterRealtimeMutation();
    refreshStudentDetailIfOpen();
    refreshMuhasebeDashboard();
  ...
  if (coll === "appointments") {
    if (op === "delete") {
      cachedAppointmentRawDocs = cachedAppointmentRawDocs.filter(function (x) {
        return x.id !== id;
      });
    } else {
      var aplain = panelMergePlainFromPayload(payload);
      var awrap = { id: id, data: function () { return aplain; } };
      if (!coachDocBelongsToRealtimeDoc(aplain)) {
        cachedAppointmentRawDocs = cachedAppointmentRawDocs.filter(function (x) {
          return x.id !== id;
        });
      } else {
        var aix = cachedAppointmentRawDocs.findIndex(function (x) {
          return x.id === id;
        });
        if (aix >= 0) cachedAppointmentRawDocs[aix] = awrap;
        else cachedAppointmentRawDocs.push(awrap);
      }
    }
    cachedAppointments = buildAppointmentList(cachedAppointmentRawDocs);
    apptCarouselOffset = 0;
  ...
  if (coll === "payments") {
    if (op === "delete") {
      cachedPayments = cachedPayments.filter(function (x) {
        return x.id !== id;
      });
    } else {
      var prow = panelMergePlainFromPayload(payload);
      prow.id = id;
      if (!coachDocBelongsToRealtimeDoc(prow)) {
        cachedPayments = cachedPayments.filter(function (x) {
          return x.id !== id;
        });
      } else {
        var pix = cachedPayments.findIndex(function (x) {
          return x.id === id;
        });
        if (pix >= 0) Object.assign(cachedPayments[pix], prow);
        else cachedPayments.push(prow);
      }
    }
    renderPaymentsTable();
    refreshMuhasebeDashboard();
  ...
  if (coll === "tests") {
    if (op === "delete") {
      cachedTests = cachedTests.filter(function (x) {
        return x.id !== id;
      });
    } else {
      var tdoc = panelMergePlainFromPayload(payload);
      tdoc.id = id;
      if (!coachDocBelongsToRealtimeDoc(tdoc)) {
        cachedTests = cachedTests.filter(function (x) {
          return x.id !== id;
        });
      } else {
        var tix = cachedTests.findIndex(function (x) {
          return x.id === id;
        });
        if (tix >= 0) Object.assign(cachedTests[tix], tdoc);
        else cachedTests.push(tdoc);
      }
    }
    renderTestsTable();
    return;
  ...
  if (coll === "coach_tasks") {
    if (op === "delete") {
      cachedCoachTasks = cachedCoachTasks.filter(function (x) {
        return x.id !== id;
      });
      var delCard = document.querySelector('[data-task-id="' + escapeHtml(String(id)) + '"]');
      if (delCard) {
        delCard.classList.add("yks-row-leave");
      }
      setTimeout(function () {
        rebuildGorevKanbanStateFromCache();
        renderGorevKanbanCards();
    ...
    var ttask = coachTaskRowFromRealtimePayload(payload, id);
    if (!coachDocBelongsToRealtimeDoc({ coach_id: ttask.coach_id, coachId: ttask.coach_id })) {
      cachedCoachTasks = cachedCoachTasks.filter(function (x) {
        return x.id !== id;
      });
    } else {
      var tkIx = cachedCoachTasks.findIndex(function (x) {
        return x.id === id;
      });
      if (tkIx >= 0) Object.assign(cachedCoachTasks[tkIx], ttask);
      else cachedCoachTasks.push(ttask);
    }
    if (op === "update") {
      var card = document.querySelector('[data-task-id="' + escapeHtml(String(id)) + '"]');
      var titleEl = card && card.querySelector(".kanban-card__text");
      var nt = cachedCoachTasks.find(function (x) {
        return x.id === id;
      });
      if (titleEl && nt) titleEl.textContent = nt.title || "";
    }
    rebuildGorevKanbanStateFromCache();
    renderGorevKanbanCards();
    refreshHpWeekIfVisible();
...
function initPanelGlobalRealtimeBridge() {
  if (stopPanelGlobalRealtime) {
    try {
      stopPanelGlobalRealtime();
    } catch (_e) {}
    stopPanelGlobalRealtime = null;
  }
  try {
    stopPanelGlobalRealtime = startPanelGlobalRealtime(function (detail) {
      applyPanelRealtimeDocEvent(detail);
    });
  } catch (e) {
    console.warn("[panel-realtime] Başlatılamadı:", e && e.message ? e.message : e);
  }
}

function softRefreshPanelData() {
...
function subscribeFirestore() {
  ...
  if (!qa || !qe || !qs || !qp || !qt || !qgt) {
    console.warn("[Appwrite] Koç kimliği çözülemedi; koleksiyon aboneliği kurulamadı.");
    showFirestoreSubscriptionBlockedMessage();
    return;
  }
  const pollOpts = { pollIntervalMs: PANEL_FIRESTORE_POLL_MS };
...
/** Appwrite `addDoc` / `updateDoc` / `setDoc` / `deleteDoc` yumuşak iptal (izin veya şema) */
function notifyIfWriteSoftFail(result, customMsg) {
  if (!isAppwriteWriteSoftFailure(result)) return false;
  dpToastApiError(
    customMsg ||
      "İşlem tamamlanamadı. Koleksiyon, indeks veya Appwrite izinlerini kontrol edin."
  );
  return true;
}

/**
 * Form gönder / kritik buton: çift tıklamayı keser, spinner gösterir.
 * @param {HTMLButtonElement|null} btn
 * @param {() => Promise<void>} fn
 */
async function runWithSubmitButtonBusy(btn, fn) {
  ...
  btn.innerHTML =
    ...
    (btn.getAttribute("data-busy-label") || "İşleniyor…") +
    "</span>";
  try {
    await fn();
  } finally {
...
async function tmWsSaveFirestoreDraft() {
  ...
  try {
    ...
    if (saveRes && saveRes.softFail) return;
    if (saveRes && saveRes.removed && saveRes.removed.length) {
      showToast("Taslak kaydedildi (" + arr.length + " soru). Şemaya uymayan alanlar atlandı.", { variant: "success" });
    } else {
      showToast("Taslak Appwrite veritabanına kaydedildi (" + arr.length + " soru).", { variant: "success" });
    }
  } catch (err) {
    console.error(err);
    showToast(err.message || String(err), { variant: "danger" });
  ...
  for (var attempt = 0; attempt < 14; attempt++) {
    try {
      ...
      if (isAppwriteWriteSoftFailure(res)) {
        return { res: res, removed: removed, softFail: true };
      }
...
/** PDF/html2canvas: çok büyük bitmap’lerde kenar üst sınırı (aşırı bellek önlemi). */
var TM_PDF_MAX_IMAGE_EDGE = 2048;
/** Gömülü img data URL — HD kalite (formül/şekil için düşürme). */
var TM_PDF_JPEG_INLINE_QUALITY = 0.98;
/** html2canvas scale: 3 = yüksek çözünürlük (Test Tasarımı PDF); sayfa bazlı yakalama bellek dostu. */
var TM_PDF_HTML2_SCALE = 3;
/** Sayfa yakalamadan önce img bekleme üst sınırı (ms) — sayfa sayfa işlendiği için agresif tutulabilir. */
var TM_PDF_PAGE_WAIT_MS = 1600;
/** A4 sayfa JPEG (html2canvas → toDataURL) — kalite / boyut dengesi. */
var TM_PDF_JPEG_PAGE_QUALITY = 0.92;
...
function tmWsDownloadPdf() {
  ...
  if (!document.getElementById("tmPdfCaptureUi")) {
    ...
    stCap.textContent =
      ...
      ".tm-pdf-live-capture,.tm-pdf-live-capture *{-webkit-font-smoothing:antialiased!important;-moz-osx-font-smoothing:grayscale!important;text-rendering:geometricPrecision!important;}";
...
function soruArsivUpdateSelectAllButtonUi() {
  var btn = document.getElementById("btnArsivSelectAll");
  var grid = document.getElementById("havuz-galeri-grid");
  if (!btn || !grid) return;
  var span = btn.querySelector(".soru-arsivi-select-all__text");
  if (!span) return;
  var boxes = grid.querySelectorAll(".soru-secim-kutu");
  if (!boxes.length) {
    span.textContent = "Tümünü Seç";
    return;
  }
  var allOn = Array.prototype.every.call(boxes, function (cb) {
    return cb.checked;
  });
  span.textContent = allOn ? "Seçimi Kaldır" : "Tümünü Seç";
}

function soruArsivDispatchCheckboxChange(el) {
  if (!el) return;
  try {
...
function soruArsivToggleSelectAllListed() {
  var grid = document.getElementById("havuz-galeri-grid");
  if (!grid || !soruArsivBulkModeActive) return;
  var boxes = grid.querySelectorAll(".soru-secim-kutu");
  if (!boxes.length) {
    showToast("Listede seçilecek soru yok.");
    return;
  }
  var allOn = Array.prototype.every.call(boxes, function (cb) {
    return cb.checked;
  });
  var wantChecked = !allOn;
  Array.prototype.forEach.call(boxes, function (cb) {
    if (cb.checked !== wantChecked) {
      cb.checked = wantChecked;
      soruArsivDispatchCheckboxChange(cb);
    }
  });
  soruArsivUpdateSelectAllButtonUi();
}
...
function soruArsivSyncBulkModeUi() {
  ...
  var selAllBtn = document.getElementById("btnArsivSelectAll");
  ...
  if (selAllBtn) {
    if (soruArsivBulkModeActive) {
      selAllBtn.removeAttribute("hidden");
      selAllBtn.style.display = "";
      soruArsivUpdateSelectAllButtonUi();
    } else {
      selAllBtn.setAttribute("hidden", "");
      selAllBtn.style.display = "none";
      var sp = selAllBtn.querySelector(".soru-arsivi-select-all__text");
      if (sp) sp.textContent = "Tümünü Seç";
    }
  }
...
function initSoruArsiviModule() {
  ...
  var btnSelAll = document.getElementById("btnArsivSelectAll");
  ...
  if (btnSelAll) btnSelAll.addEventListener("click", soruArsivToggleSelectAllListed);
  var grid = document.getElementById("havuz-galeri-grid");
  if (grid) {
    grid.addEventListener("change", function (ev) {
      var t = ev.target;
      if (t && t.classList && t.classList.contains("soru-secim-kutu")) {
        soruArsivUpdateSelectAllButtonUi();
      }
    });
  }
...
function navigateTo(view) {
  ...
  if (previous === "gelen-sorular" && view !== "gelen-sorular") {
    try {
      destroyCoachInboxChatView();
    } catch (_ch) {}
  }
  var wasTm =
    previous === "testmaker" ||
  ...
  currentView = view;
  document.body.classList.toggle("body--gorusme-cockpit", view === "gorusme-odasi");
  try {
    var appShell = document.querySelector(".app");
    if (appShell && view === "gorusme-odasi") appShell.classList.add("app--sidebar-readable");
    var locUrl = new URL(window.location.href);
    if (view === "gorusme-odasi") {
      locUrl.searchParams.set("view", "gorusme-odasi");
      history.replaceState(null, "", locUrl.pathname + locUrl.search + locUrl.hash);
    } else if (locUrl.searchParams.get("view") === "gorusme-odasi") {
      locUrl.searchParams.delete("view");
      history.replaceState(null, "", locUrl.pathname + (locUrl.search || "") + locUrl.hash);
    }
  } catch (e) {}
  ...
  var daLi = document.querySelector(".sidebar__item--deneme-analizi");
  var daAcc = document.getElementById("sidebarDanaToggle");
  if (daLi && daAcc) {
    var inDa =
      view === "deneme-analiz-denemeler" ||
      view === "deneme-analiz-takvim" ||
      ...
      view === "optik-okuyucu" ||
      view === "karne";
    daLi.classList.toggle("sidebar__item--dana-open", inDa);
    daAcc.classList.toggle("sidebar__link--active", inDa);
    daAcc.setAttribute("aria-expanded", inDa ? "true" : "false");
  }
  var mrLi = document.querySelector(".sidebar__item--mr");
  var mrAcc = document.getElementById("sidebarMrToggle");
  if (mrLi && mrAcc) {
    var inMr = view === "mr-konu" || view === "mr-soru" || view === "mr-deneme";
    mrLi.classList.toggle("sidebar__item--mr-open", inMr);
  ...
  if (simLi && simAcc) {
    var inSim =
      view === "hedef-simulator" ||
      view === "net-sihirbazi" ||
      view === "yks-puan" ||
      view === "tercih-sihirbazi";
  ...
  if (view === "denemeler" || view === "deneme-analiz-denemeler") {
    renderExamsFullPage();
    initDenemeAnalizPage();
    bindDenemeAnalizForm();
    if (view === "deneme-analiz-denemeler" && typeof window.initDenemePlanlamaPage === "function") {
      try {
        window.initDenemePlanlamaPage();
      } catch (e) {
        console.warn("[deneme-planlama]", e);
      }
    }
  }
  ...
  if (view === "optik-okuyucu" || view === "deneme-analiz-optik") {
    var od = document.getElementById("optikImportExamDate");
    if (od && !od.value) od.value = new Date().toISOString().slice(0, 10);
  }
  if (view === "karne" || view === "deneme-analiz-karne") {
    refreshKarneKpis();
    renderKarneReport();
  }
  if (view === "deneme-analiz-karne" && typeof window.initDenemeAnaliziPremium === "function") {
    try {
      window.initDenemeAnaliziPremium();
    } catch (e) {
      console.warn("[deneme-analizi]", e);
    }
  }
  if (view === "mr-konu" || view === "mr-soru" || view === "mr-deneme") {
    try {
      initMrCockpit(view);
    } catch (e) {
  ...
  if (view === "gelen-sorular") {
    initCoachInboxChatView({
      getCoachId: getCoachId,
      getStudents: function () {
        return cachedStudents.slice();
      },
      showToast: showToast,
      resolveStudentAvatarUrl: function (st) {
        var name = st.name || st.studentName || "Öğrenci";
        var raw = st.avatarUrl;
        if (raw && /^https?:\/\//i.test(String(raw).trim())) return String(raw).trim();
        return buildStudentAvatarUrl(name, st.gender);
      },
    });
  }
...
/** Otomatik kırpıcı — soru görseli düzenleme modalı (canvas seçim + dosya) */
var tmAutoCropEdit = {
  idx: -1,
  img: null,
  natW: 0,
...
function tmAutoCropFillDersKonuFromExam() {
  ...
  var prevD = dersEl.value;
  dersEl.innerHTML = '<option value="">— Ders seçiniz —</option>';
  dersler.forEach(function (d) {
    var o = document.createElement("option");
    o.value = d;
    o.textContent = d;
  ...
  if (dersler.indexOf(prevD) !== -1) dersEl.value = prevD;
  function fillKonu() {
    var d = dersEl.value;
    var list = (bag[d] || []).slice();
    konuEl.innerHTML = '<option value="">— Konu seçiniz —</option>';
    if (!list.length) return;
    list.forEach(function (t) {
      var o = document.createElement("option");
      o.value = t;
...
function tmAutoCropRenderDebug(data) {
  var dm = document.getElementById("tmAutoCropDebugMount");
  if (!dm) return;
  dm.innerHTML = "";
  if (!data || !Array.isArray(data.debug_pages) || !data.debug_pages.length) return;
  var leg = document.createElement("p");
  leg.className = "text-xs text-slate-600 mt-3 mb-2";
  leg.innerHTML =
    "Hata ayıklama: <strong>yeşil kalın</strong> regex soru satırı; <strong>turkuaz</strong> boşluk ipucu; <strong>mavi ince</strong> tüm metin satırı; <strong>kırmızı</strong> kesime girmeyen uzun satır; <strong>yeşil alan</strong> kesilen soru; <strong>mor</strong> sütun ayracı.";
  dm.appendChild(leg);
  data.debug_pages.forEach(function (p) {
    var h = document.createElement("div");
    h.className = "mt-2 font-semibold text-xs text-violet-800";
    h.textContent = "Sayfa " + (p.page || "?");
    dm.appendChild(h);
    var img = document.createElement("img");
    img.alt = "Debug sayfa " + (p.page || "");
    img.src = String(p.overlay_base64 || "");
    img.style.cssText = "max-width:100%;border:1px solid #e2e8f0;border-radius:8px;display:block";
    dm.appendChild(img);
  });
}

function tmAutoCropEditCanvasPos(cv, ev) {
  var r = cv.getBoundingClientRect();
  var sx = cv.width / (r.width || 1);
  var sy = cv.height / (r.height || 1);
  return {
...
function tmAutoCropEditRedraw() {
  var cv = document.getElementById("tmAutoCropEditCanvas");
  if (!cv || !tmAutoCropEdit.img || !tmAutoCropEdit.img.complete) return;
  var ctx = cv.getContext("2d");
  ctx.clearRect(0, 0, cv.width, cv.height);
  ctx.drawImage(tmAutoCropEdit.img, 0, 0, cv.width, cv.height);
  var x0 = Math.min(tmAutoCropEdit.x0, tmAutoCropEdit.x1);
  var y0 = Math.min(tmAutoCropEdit.y0, tmAutoCropEdit.y1);
  var x1 = Math.max(tmAutoCropEdit.x0, tmAutoCropEdit.x1);
  var y1 = Math.max(tmAutoCropEdit.y0, tmAutoCropEdit.y1);
  if (tmAutoCropEdit.hasUserSel && x1 - x0 > 2 && y1 - y0 > 2) {
...
function tmAutoCropCloseEditModal() {
  var modal = document.getElementById("tmAutoCropEditModal");
  if (modal) modal.setAttribute("hidden", "");
  tmAutoCropEdit.dragging = false;
  tmAutoCropEdit.img = null;
  tmAutoCropEdit.idx = -1;
  if (tmAutoCropEdit.escHandler) {
    document.removeEventListener("keydown", tmAutoCropEdit.escHandler);
    tmAutoCropEdit.escHandler = null;
  }
}

function tmAutoCropOpenEditModal(idx) {
  if (isNaN(idx) || !tmAutoCropResults[idx]) return;
  var raw = String(tmAutoCropResults[idx].imageBase64 || "").trim();
  if (!raw) {
    showToast("Bu soru için görsel bulunamadı.");
    return;
  }
  var modal = document.getElementById("tmAutoCropEditModal");
  var cv = document.getElementById("tmAutoCropEditCanvas");
  if (!modal || !cv) {
    showToast("Düzenleme penceresi yüklenemedi.");
    return;
  }
  tmAutoCropEdit.idx = idx;
  tmAutoCropEdit.hasUserSel = false;
  tmAutoCropEdit.dragging = false;
  var im = new Image();
  im.onload = function () {
    tmAutoCropEdit.img = im;
    var maxW = 880;
    var iw = im.naturalWidth;
    var ih = im.naturalHeight;
    tmAutoCropEdit.natW = iw;
    tmAutoCropEdit.natH = ih;
    var dw = iw;
    var dh = ih;
    if (dw > maxW) {
      dh = Math.round((ih * maxW) / iw);
      dw = maxW;
    }
    cv.width = dw;
    cv.height = dh;
    tmAutoCropEdit.x0 = 0;
    tmAutoCropEdit.y0 = 0;
    tmAutoCropEdit.x1 = dw;
    tmAutoCropEdit.y1 = dh;
    tmAutoCropEditRedraw();
    modal.removeAttribute("hidden");
    tmAutoCropEdit.escHandler = function (e) {
      if (e.key === "Escape") tmAutoCropCloseEditModal();
    };
    document.addEventListener("keydown", tmAutoCropEdit.escHandler);
  };
  im.onerror = function () {
    showToast("Görsel yüklenemedi.");
    tmAutoCropEdit.idx = -1;
...
function tmAutoCropEditApplySelection() {
  var cv = document.getElementById("tmAutoCropEditCanvas");
  if (!cv || tmAutoCropEdit.idx < 0 || !tmAutoCropEdit.img || !tmAutoCropResults[tmAutoCropEdit.idx]) return;
  var nw = tmAutoCropEdit.natW;
  var nh = tmAutoCropEdit.natH;
  var cw = cv.width;
  var ch = cv.height;
  var x0 = Math.min(tmAutoCropEdit.x0, tmAutoCropEdit.x1);
  var y0 = Math.min(tmAutoCropEdit.y0, tmAutoCropEdit.y1);
  ...
  var rh = y1 - y0;
  if (rw < 8 || rh < 8) {
    showToast("Geçerli bir alan seçin (sürükleyin).");
    return;
  }
  var sx = Math.max(0, Math.min(nw - 1, Math.round((x0 / cw) * nw)));
  var sy = Math.max(0, Math.min(nh - 1, Math.round((y0 / ch) * nh)));
  var sw = Math.max(1, Math.round((rw / cw) * nw));
  var sh = Math.max(1, Math.round((rh / ch) * nh));
  if (sx + sw > nw) sw = nw - sx;
  if (sy + sh > nh) sh = nh - sy;
  var out = document.createElement("canvas");
  out.width = sw;
  out.height = sh;
  out.getContext("2d").drawImage(tmAutoCropEdit.img, sx, sy, sw, sh, 0, 0, sw, sh);
  var dataUrl = out.toDataURL("image/png");
  tmAutoCropResults[tmAutoCropEdit.idx].imageBase64 = dataUrl;
  tmAutoCropCloseEditModal();
  tmAutoCropRenderGrid();
  showToast("Soru görseli güncellendi.");
}

function tmAutoCropBindEditModal() {
  var modal = document.getElementById("tmAutoCropEditModal");
  var cv = document.getElementById("tmAutoCropEditCanvas");
  var applyBtn = document.getElementById("tmAutoCropEditApplyBtn");
  var cancelBtn = document.getElementById("tmAutoCropEditCancelBtn");
  var fileBtn = document.getElementById("tmAutoCropEditFileBtn");
  var fileInp = document.getElementById("tmAutoCropEditFileInput");
  if (!modal || !cv) return;
  modal.addEventListener("click", function (ev) {
    var t = ev.target;
    if (t && t.closest && t.closest("[data-tm-auto-crop-edit-dismiss]")) tmAutoCropCloseEditModal();
  });
  if (cancelBtn) cancelBtn.addEventListener("click", tmAutoCropCloseEditModal);
  if (applyBtn) applyBtn.addEventListener("click", tmAutoCropEditApplySelection);
  if (fileBtn && fileInp) {
  ...
  if (fileInp) {
    fileInp.addEventListener("change", function () {
      ...
      if (!f || tmAutoCropEdit.idx < 0) return;
      var r = new FileReader();
      r.onload = function () {
        var url = r.result;
        tmAutoCropResults[tmAutoCropEdit.idx].imageBase64 = url;
        tmAutoCropRenderGrid();
        tmAutoCropCloseEditModal();
        showToast("Görsel değiştirildi. Gerekirse Düzenle ile tekrar kırpın.");
      };
      r.readAsDataURL(f);
      fileInp.value = "";
    });
  ...
  function onMove(ev) {
    if (!tmAutoCropEdit.dragging) return;
    var p = tmAutoCropEditCanvasPos(cv, ev);
    tmAutoCropEdit.x1 = Math.max(0, Math.min(cv.width, p.x));
    tmAutoCropEdit.y1 = Math.max(0, Math.min(cv.height, p.y));
    if (Math.abs(tmAutoCropEdit.x1 - tmAutoCropEdit.x0) > 4 || Math.abs(tmAutoCropEdit.y1 - tmAutoCropEdit.y0) > 4) {
      tmAutoCropEdit.hasUserSel = true;
    }
    tmAutoCropEditRedraw();
  }
  function onUp() {
  ...
  cv.addEventListener("mousedown", function (ev) {
    if (!tmAutoCropEdit.img) return;
    ev.preventDefault();
    var p = tmAutoCropEditCanvasPos(cv, ev);
    tmAutoCropEdit.dragging = true;
    tmAutoCropEdit.x0 = tmAutoCropEdit.x1 = Math.max(0, Math.min(cv.width, p.x));
    tmAutoCropEdit.y0 = tmAutoCropEdit.y1 = Math.max(0, Math.min(cv.height, p.y));
  });
...
function tmAutoCropRenderGrid() {
  ...
  visible.forEach(function (entry) {
    ...
    editBtn.type = "button";
    editBtn.className = "btn-edit-question tm-auto-crop-item__edit";
    editBtn.setAttribute("data-tm-auto-crop-edit", String(idx));
    editBtn.setAttribute("title", "Görseli düzenle veya yeniden kırp");
    editBtn.innerHTML = '<i class="fa-solid fa-pen" aria-hidden="true"></i> Düzenle';
...
async function tmAutoCropRun() {
  ...
  var dbgEl = document.getElementById("tmAutoCropDebug");
  fd.append("debug", dbgEl && dbgEl.checked ? "1" : "0");
...
function initTmAutoCropperModule() {
  ...
  var grid = document.getElementById("tmAutoCropGrid");
  var pageFilter = document.getElementById("tmAutoCropPageFilter");
  var examEl = document.getElementById("tmAutoCropExam");
  var bulkDers = document.getElementById("tmAutoCropBulkDers");
  var applyBulkDers = document.getElementById("tmAutoCropApplyBulkDers");
  var applyToolbarMeta = document.getElementById("tmAutoCropApplyToolbarMeta");
  ...
  if (examEl) examEl.addEventListener("change", tmAutoCropFillDersKonuFromExam);
  tmAutoCropFillDersKonuFromExam();
  if (pageFilter) {
  ...
  if (applyToolbarMeta) {
    applyToolbarMeta.addEventListener("click", function () {
      ...
      if (!d && !k) {
        showToast("Önce üstte ders veya konu seçin.");
        return;
      }
      var p = (pageFilter && pageFilter.value) || "all";
      tmAutoCropResults.forEach(function (x) {
        if (p === "all" || String(x.page || 1) === String(p)) {
          if (d) x.editedDers = d;
          if (k) x.editedKonu = k;
        }
      });
      tmAutoCropRenderGrid();
      showToast("Üstteki ders/konu uygulandı.");
    });
  }
  if (selectAllBtn) {
  ...
  grid.addEventListener("click", function (ev) {
    ...
    var editQ = ev.target && ev.target.closest && ev.target.closest("[data-tm-auto-crop-edit]");
    if (editQ) {
      var idxE = parseInt(editQ.getAttribute("data-tm-auto-crop-edit"), 10);
      tmAutoCropOpenEditModal(idxE);
      return;
    }
...
function initNavigation() {
  ...
  function bindSidebarTmNavClicks(container) {
    ...
    container.addEventListener("click", function (e) {
      ...
      if (action === "auto-cropper") navigateTo("auto-cropper");
...
function hpBuildOneWeekGrid(gridEl, weekMonday, opts) {
  ...
  for (var i = 0; i < 7; i++) {
    ...
    col.className = "hp-day" + (readonly ? " hp-day--readonly" : "");
    col.setAttribute("data-hp-day-iso", iso);
    var innerDrop = readonly
      ? '<div class="hp-day__static" data-hp-prev="' +
        iso +
        '" data-hp-day-idx="' +
        i +
        '"></div>'
...
function hpTaskTypeLabel(meta) {
  var gt = meta && meta.gorevTipi ? meta.gorevTipi : "";
  if (gt && HP_GOREV_TIPI_LABELS[gt]) return HP_GOREV_TIPI_LABELS[gt];
  var tt = meta && meta.taskType ? meta.taskType : "konu";
  var typeMap = { konu: "📖 Konu", soru: "📝 Soru", deneme: "🎯 Deneme" };
  return typeMap[tt] || "Görev";
}
...
function hpPopulateSubjectTopics() {
  ...
  if (!sub || !top) return;
  var keys = hpGetSubjectKeysFromMufredat();
  sub.innerHTML =
  ...
  function fillTopics() {
    var sk = sub.value;
    var arr = sk ? hpTopicsForSubject(sk) : [];
    top.innerHTML =
...
function hpOpenWeeklyTaskModal(tool, dateIso) {
  ...
  var gorevTipiEl = document.getElementById("hpMtGorevTipi");
  ...
  var tq = document.getElementById("hpMtTargetQuestions");
  var est = document.getElementById("hpMtEstimatedMinutes");
  var sub = document.getElementById("hpWeeklyTaskModalSubtitle");
  if (toolEl) toolEl.value = tool;
  if (gorevTipiEl) gorevTipiEl.value = hpMapLegacyToolToGorevTipi(tool);
  ...
  if (tq) tq.value = "";
  if (est) est.value = "";
  if (sub) sub.textContent = "Merkezi YKS müfredatı · " + dateIso;
...
function hpOpenWeeklyTaskModalForEdit(taskId) {
  ...
  var meta = task.hpMeta || hpParseMetaFromDescription(task.description).meta;
  hpEditingTaskId = taskId;
  hpPendingDrop = {
    tool: meta && meta.taskType ? meta.taskType : "konu",
    dateIso: String(task.dueDate || "").slice(0, 10),
  };
  hpPopulateSubjectTopics();
  var toolEl = document.getElementById("hpMtToolType");
  var gorevTipiEl = document.getElementById("hpMtGorevTipi");
  ...
  var tt = document.getElementById("hpWeeklyTaskModalTitle");
  if (toolEl) toolEl.value = (meta && meta.taskType) || "konu";
  if (gorevTipiEl) gorevTipiEl.value = hpGorevTipiFromMeta(meta);
  if (dateEl) dateEl.value = String(task.dueDate || "").slice(0, 10);
  if (vid) vid.value = meta && meta.videoUrl ? meta.videoUrl : "";
  if (res) res.value = meta && meta.resource ? meta.resource : "";
  if (notes) notes.value = meta && meta.notes ? meta.notes : "";
  if (tq) tq.value = meta && meta.targetQuestions != null ? String(meta.targetQuestions) : "";
  if (est) est.value = meta && meta.estimatedMinutes != null ? String(meta.estimatedMinutes) : "";
  if (subEl) {
  ...
  if (topEl && meta && meta.topic) {
    topEl.value = meta.topic;
  }
  if (subti) {
...
function hpSaveWeeklyTaskFromModal() {
  ...
  var legacyTool = (toolEl && toolEl.value) || (hpPendingDrop && hpPendingDrop.tool) || "konu";
  if (toolEl && !toolEl.value && hpPendingDrop && hpPendingDrop.tool) toolEl.value = hpPendingDrop.tool;
  var gorevTipi = gorevTipiEl && String(gorevTipiEl.value || "").trim();
  if (!gorevTipi) gorevTipi = hpMapLegacyToolToGorevTipi(legacyTool);
  var meta = hpBuildTaskMetaFromForm({
    gorevTipi: gorevTipi,
    topic: k,
    targetQuestions: tq ? tq.value : "",
    resource: res ? res.value : "",
    videoUrl: vid ? vid.value : "",
    notes: notes ? notes.value : "",
    estimatedMinutes: est ? est.value : "",
  });
  var title = hpBuildCoachTaskTitle(s, meta);
  var desc = hpSerializeWeeklyMeta(meta);
  var sname = stu ? String(stu.name || stu.studentName || "").trim() : "";

  if (hpEditingTaskId) {
    updateDoc(doc(db, "coach_tasks", hpEditingTaskId), {
      title: title,
      description: desc,
      subject: s,
      ...
      .then(function (r) {
        ...
        showToast("Görev Appwrite üzerinde güncellendi.", { variant: "success" });
      ...
      .catch(function (e) {
        console.error(e);
        showToast("Güncellenemedi.", { variant: "danger" });
      });
    return;
  }
  ...
  addDoc(collection(db, "coach_tasks"), {
    ...
    .then(function (r) {
      ...
      hpPendingDrop = null;
      closeModal("hpWeeklyTaskModal");
      showToast("Görev Appwrite'a kaydedildi.", { variant: "success" });
      void saveOgrenciVerisiBridge({ silent: true, studentIdOverride: hpSelectedStudentId }).catch(function (e) {
        console.warn(e);
      });
    })
    .catch(function (e) {
      console.error(e);
      showToast("Kayıt başarısız.", { variant: "danger" });
    });
}

function hpCarryIncompleteToCurrentWeek() {
  ...
  prevMon.setDate(prevMon.getDate() - 7);
  var prevEnd = new Date(prevMon.getTime());
  prevEnd.setDate(prevEnd.getDate() + 6);
  var prevStartIso = hpLocalISODate(prevMon);
  var prevEndIso = hpLocalISODate(prevEnd);
  var prevMonMs = prevMon.getTime();

  var toCopy = (cachedCoachTasks || []).filter(function (t) {
    if (String(t.studentId || "") !== sid) return false;
    var d = String(t.dueDate || "").slice(0, 10);
    if (d < prevStartIso || d > prevEndIso) return false;
    if ((t.column || "todo") === "done") return false;
    return true;
  });
  if (!toCopy.length) {
    showToast("Aktarılacak yapılmamış görev yok.");
    return;
  }
  var stu = cachedStudents.find(function (x) {
    return x.id === sid;
  ...
  toCopy.forEach(function (t) {
    chain = chain.then(function () {
      var meta = t.hpMeta || hpParseMetaFromDescription(t.description).meta;
      if (!meta || meta.v !== 1) {
        meta = hpBuildTaskMetaFromForm({
          gorevTipi: "konu_calisma",
          topic: "",
          targetQuestions: "",
          resource: "",
          videoUrl: "",
          estimatedMinutes: "",
          notes:
            String(t.description || "").trim().charAt(0) === "{"
              ? ""
      ...
      var dStr = String(t.dueDate || "").slice(0, 10);
      var due = new Date(dStr + "T12:00:00");
      var dayIdx = Math.round((due.getTime() - prevMonMs) / 86400000);
      if (dayIdx < 0 || dayIdx > 6) dayIdx = 0;
      var newDt = new Date(curMon.getTime());
      newDt.setDate(newDt.getDate() + dayIdx);
      var newIso = hpLocalISODate(newDt);
      var title = hpBuildCoachTaskTitle(t.subject, meta);
      var desc = hpSerializeWeeklyMeta(meta);
      return addDoc(collection(db, "coach_tasks"), {
        ...
        if (isAppwriteWriteSoftFailure(r)) {
          notifyIfWriteSoftFail(r);
          throw new Error("__hp_carry_abort");
        }
        return r;
      });
    });
  ...
  chain
    .then(function () {
      showToast(toCopy.length + " görev bu haftaya kopyalandı (Appwrite).", { variant: "success" });
      void saveOgrenciVerisiBridge({ silent: true, studentIdOverride: sid }).catch(function (e) {
        console.warn(e);
      });
    })
    .catch(function (e) {
      if (e && e.message === "__hp_carry_abort") return;
      console.error(e);
      showToast("Aktarım tamamlanamadı.", { variant: "danger" });
    });
}
...
var kglListLoading = false;

function kglSavedAtMs(data) {
  var raw = data && data.saved_at;
...
function kglParseBodyForCard(html) {
  var topic = "";
  var summary = "";
  if (!html) return { topic: "—", summary: "" };
  try {
    var wrap = document.createElement("div");
    wrap.innerHTML = html;
    var tl = wrap.querySelector(".kgl-topic-line");
    var bd = wrap.querySelector(".kgl-body");
    if (tl) topic = (tl.textContent || "").replace(/^Konu:\s*/i, "").trim();
    if (bd) summary = (bd.textContent || "").trim();
    else summary = (wrap.textContent || "").trim();
  } catch (_e) {
    summary = String(html)
      .replace(/<[^>]+>/g, "")
      .trim();
  ...
  if (!topic) topic = "—";
  return { topic: topic, summary: summary.slice(0, 220) };
}

function kglFormatDateTr(isoOrDate) {
  if (!isoOrDate) return "—";
  var d;
  if (isoOrDate instanceof Date) d = isoOrDate;
  else {
    var t = Date.parse(String(isoOrDate));
...
function kglBuildBodyHtml(topic, detailPlain) {
  return (
    '<p class="kgl-topic-line"><strong>Konu:</strong> ' +
    escapeHtml(topic) +
    "</p>" +
    '<div class="kgl-body">' +
    escapeHtml(detailPlain).replace(/\n/g, "<br>") +
    "</div>"
  );
}

function kglCollectMeetingRows(snap) {
  var rows = [];
...
function kglDedupeMeetingRows(rows) {
  var seen = Object.create(null);
  var out = [];
  rows.forEach(function (r) {
    if (seen[r.id]) return;
...
/** coach_id sorgusu indeks yoksa boş dönebilir; o zaman öğrenci başına student_id ile yedeklenir. */
function kglFetchMeetingLogsByStudents(cid) {
  if (!cachedStudents || !cachedStudents.length) return Promise.resolve([]);
  return Promise.all(
    cachedStudents.map(function (s) {
      return getDocs(
        query(collection(db, APPWRITE_COLLECTION_MEETING_LOGS), where("student_id", "==", s.id))
      );
    })
  ).then(function (snaps) {
    var merged = [];
...
function kglRenderMeetingCards(root, rows) {
  rows.sort(function (a, b) {
    return kglSavedAtMs(b.data) - kglSavedAtMs(a.data);
  });
  if (!rows.length) {
    root.innerHTML =
      '<div class="kgl-empty">Henüz kayıtlı görüşme notu yok. <strong>+ Not ekle</strong> ile ilk kaydı oluşturun.</div>';
    return;
  }
  root.innerHTML = rows
    .map(function (row) {
      var data = row.data;
      var parsed = kglParseBodyForCard(String(data.body_html || data.bodyHtml || ""));
      var name = String(data.student_name || "").trim() || "Öğrenci";
      var dateLabel = kglFormatDateTr(data.saved_at);
      return (
        '<article class="kgl-card" role="article">' +
        '<div class="kgl-card__top">' +
        '<strong class="kgl-card__name">' +
        escapeHtml(name) +
        "</strong>" +
        '<span class="kgl-card__date">' +
        escapeHtml(dateLabel) +
        "</span>" +
        "</div>" +
        '<p class="kgl-card__topic">Konu: ' +
        escapeHtml(parsed.topic) +
        "</p>" +
        '<p class="kgl-card__summary">' +
...
function refreshKoclukGorusmeList() {
  var root = document.getElementById("yksKoclukTimeline");
  if (!root) return;
  var cid = getCoachId();
  if (!cid) {
    root.innerHTML = '<p class="kgl-empty">Kullanıcı oturumu yükleniyor…</p>';
    return;
  }
  if (kglListLoading) return;
  kglListLoading = true;
  root.innerHTML =
    '<p class="kgl-empty"><i class="fa-solid fa-spinner fa-spin" aria-hidden="true"></i> Yükleniyor…</p>';
  getDocs(query(collection(db, APPWRITE_COLLECTION_MEETING_LOGS), where("coach_id", "==", cid)))
    .then(function (snap) {
      var rows = kglCollectMeetingRows(snap);
      if (rows.length) return rows;
      return kglFetchMeetingLogsByStudents(cid);
    })
    .then(function (rows) {
      rows = kglDedupeMeetingRows(rows || []);
      kglRenderMeetingCards(root, rows);
    })
    .catch(function (err) {
      console.error(err);
      root.innerHTML =
        '<p class="kgl-empty kgl-empty--err">Görüşme notları yüklenemedi. Lütfen bağlantınızı kontrol edin.</p>';
    })
    .finally(function () {
      kglListLoading = false;
    });
}

function kglOpenModal() {
  fillStudentSelects();
  var form = document.getElementById("formKoclukGorusme");
  if (form) form.reset();
  var di = document.getElementById("kglModalDate");
  if (di) di.value = new Date().toISOString().slice(0, 10);
  openModal("koclukGorusmeModal");
}

function kglSubmitModal(ev) {
  ev.preventDefault();
  var cid = getCoachId();
  var sel = document.getElementById("kglModalStudent");
  var dateIn = document.getElementById("kglModalDate");
  var topicIn = document.getElementById("kglModalTopic");
  var detailIn = document.getElementById("kglModalDetail");
  if (!cid || !sel || !dateIn || !topicIn || !detailIn) return;
  var sid = String(sel.value || "").trim();
  if (!sid) {
    showToast("Lütfen bir öğrenci seçin.");
    return;
  }
  var topic = String(topicIn.value || "").trim();
  if (!topic) {
    showToast("Görüşme konusu zorunludur.");
    return;
  }
  var detail = String(detailIn.value || "").trim();
  if (!detail) {
    showToast("Not detayı zorunludur.");
    return;
  }
  var st = cachedStudents.find(function (x) {
    return x.id === sid;
  ...
  var sname =
    st && (st.name || st.studentName)
      ? String(st.name || st.studentName)
      : "Öğrenci";
  var dateStr = dateIn.value || new Date().toISOString().slice(0, 10);
  var meetingDate = new Date(dateStr + "T12:00:00");
  if (isNaN(meetingDate.getTime())) meetingDate = new Date();
  var body_html = kglBuildBodyHtml(topic, detail);
  addDoc(collection(db, APPWRITE_COLLECTION_MEETING_LOGS), {
    coach_id: cid,
    student_id: sid,
    student_name: sname.slice(0, 500),
    body_html: body_html,
    saved_at: meetingDate,
  })
    .then(function (r) {
      if (notifyIfWriteSoftFail(r)) return;
      showToast("Görüşme notu kaydedildi.", { variant: "success" });
    ...
    .catch(function (err) {
      console.error(err);
      showToast(err && err.message ? err.message : "Kayıt başarısız.", { variant: "danger" });
    });
}

function initKoclukGorusmelerOnce() {
  var btn = document.getElementById("btnKoclukNotEkle");
  if (btn && !btn.dataset.boundKgl) {
    btn.dataset.boundKgl = "1";
    btn.addEventListener("click", function () {
      kglOpenModal();
    });
  }
  var form = document.getElementById("formKoclukGorusme");
  if (form && !form.dataset.boundKgl) {
    form.dataset.boundKgl = "1";
    form.addEventListener("submit", kglSubmitModal);
  }
}

function initYksFeaturePages() {
  initKoclukGorusmelerOnce();
...
function kkSubscribeStudentAssignments(studentId) {
  ...
  kkAssignUnsub = onSnapshot(
    ...
    function (snap) {
      ...
      snap.forEach(function (d) {
        var x = d.data();
        var rawT = x.topics_json;
        var parsedTopics = [];
        if (typeof rawT === "string" && rawT) {
          try {
            parsedTopics = JSON.parse(rawT);
          } catch (_e) {}
        } else if (Array.isArray(x.topics)) {
          parsedTopics = x.topics;
        }
        kkAssignedBooksCache.push({
          ...
          topics: kkNormalizeFirestoreTopics(parsedTopics),
...
function bindKutuphanemFormOnce() {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    try {
      var _wrKaynak = await addDoc(collection(db, "kaynaklar"), {
        coach_id: cid,
        title: title,
        subject: subject,
        totalPages: total,
      ...
      if (notifyIfWriteSoftFail(_wrKaynak)) return;
      showToast("Kitap kütüphaneye eklendi.", { variant: "success" });
      form.reset();
    } catch (err) {
      console.error(err);
...
function kkSetTopicStatus(studentId, bookId, topicIndex, status) {
  ...
  updateDoc(doc(db, APPWRITE_COLLECTION_ATANAN_KAYNAKLAR, bookId), {
    topics_json: JSON.stringify(book.topics || []),
  })
    .then(function (r) {
      if (notifyIfWriteSoftFail(r)) return;
    })
    .catch(function (e) {
      console.error(e);
      showToast("Konu durumu kaydedilemedi.", { variant: "danger" });
    });
}
...
function kkAddPerformance(studentId, bookId, d, y) {
  ...
  updateDoc(doc(db, APPWRITE_COLLECTION_ATANAN_KAYNAKLAR, bookId), {
    ...
    .catch(function (e) {
      console.error(e);
      showToast("Performans kaydedilemedi.", { variant: "danger" });
    });
}
...
async function kkAssignBookFromLibrary() {
  ...
  try {
    var _wrKkAsn = await addDoc(collection(db, APPWRITE_COLLECTION_ATANAN_KAYNAKLAR), {
      student_id: kkSelectedStudentId,
      coach_id: getCoachId(),
      libraryId: lib.id,
      title: lib.title,
      subject: lib.subject,
      ...
      topics_json: JSON.stringify([{ id: kkNewTopicId(), name: "Genel çalışma", status: 0 }]),
      correctTotal: 0,
      wrongTotal: 0,
      assignedAt: serverTimestamp(),
    });
    if (notifyIfWriteSoftFail(_wrKkAsn)) return;
    showToast("Kaynak öğrenciye eklendi.", { variant: "success" });
...
async function deleteAssignedResource(documentId) {
  ...
  try {
    var _wrKkDel = await deleteDoc(doc(db, APPWRITE_COLLECTION_ATANAN_KAYNAKLAR, documentId));
    if (notifyIfWriteSoftFail(_wrKkDel)) return;
...
function initAllButtons() {
  ...
  document.getElementById("btnSeeAllExams") &&
    document.getElementById("btnSeeAllExams").addEventListener("click", function () {
      navigateTo("deneme-analiz-denemeler");
    });
    ...
    document.querySelectorAll(".sd-erp-tab").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        if (idx === "1" && currentStudentDetailId) {
          setTimeout(function () {
            void renderStudentDetailEliteAnalytics(currentStudentDetailId);
          }, 60);
        }
      });
    });
  ...
  document.getElementById("btnLogout") &&
    document.getElementById("btnLogout").addEventListener("click", function (e) {
      ...
      try {
        ...
        localStorage.removeItem("dp_appwrite_user_id");
        localStorage.removeItem("yksRole");
        localStorage.removeItem("yksStudentName");
        localStorage.removeItem("yksCoachId");
        delete window.DerecePanelAuth;
      } catch (_ls) {}
...
window.YKSPanel = {
  getMockTasks: function () {
    return (cachedCoachTasks || []).slice();
  },
  getCachedStudents: function () {
    return (cachedStudents || []).slice();
  },
  getCachedExams: function () {
    return (cachedExams || []).slice();
  },
  getCachedCoachTasks: function () {
    return (cachedCoachTasks || []).slice();
  },
  /** Sayfa yenilemeden Appwrite snapshot + Realtime aboneliklerini yeniden kurar */
  softRefreshData: softRefreshPanelData,
  getCoachId: getCoachId,
  navigate: navigateTo,
   ...
   */
  runAsyncWithButton: runWithSubmitButtonBusy,
...
function bootstrapKocPanelAfterAuth() {
  ...
  try {
    localStorage.removeItem("yks_mock_tasks_v1");
  } catch (e) {}
  applySpaInitialShellState();
  configureDashboardCharts({
    coachQuery: coachQuery,
    getDocs: getDocs,
    normalizeAppointmentDoc: normalizeAppointmentDoc,
    appointmentSortTime: appointmentSortTime,
  });
  configureDashboardInsights({
    getStudents: function () {
    ...
    getMeetingLogs: getDashboardMeetingLogsSnapshot,
    appointmentSortTime: appointmentSortTime,
  ...
  configureDashboardActivities({
    ...
    getDocs: getDocs,
    escapeHtml: escapeHtml,
    APPWRITE_COLLECTION_MEETING_LOGS: APPWRITE_COLLECTION_MEETING_LOGS,
    APPWRITE_COLLECTION_EXAM_RESULTS: APPWRITE_COLLECTION_EXAM_RESULTS,
  });
  initSidebar();
  initNavigation();
  initModals();
...
function finalizeKocPanelAdmission(profile, user) {
  var imp = false;
  try {
  ...
  return getAppSettings()
    .then(function (appSettings) {
      var maint = !!(appSettings && appSettings.maintenance);
    ...
    .catch(function (err) {
      console.warn("[koc-panel] finalizeKocPanelAdmission getAppSettings", err);
      var uname = profile.username;
      if (!uname && user.email) uname = user.email.split("@")[0];
      try {
        localStorage.setItem("currentUser", (uname || "").trim());
...
function loadKocPanelForUser(user) {
  ...
  var gated = getLastCoachGateResult();
  if (gated && gated.compatUser && gated.compatUser.uid === user.uid && gated.profile) {
    finalizeKocPanelAdmission(gated.profile, user);
    return;
  }
  fetchAppwriteUserProfile({ uid: user.uid, email: user.email || "" }, "")
    .then(function (profile) {
      finalizeKocPanelAdmission(profile, user);
    })
    .catch(function (err) {
      console.error("[koc-panel] fetchAppwriteUserProfile", err);
      signOut(auth).finally(function () {
        window.location.replace(DP_LOGIN_PATH);
      });
    });
...
setTimeout(function () {
  ...
  verifyAppwriteAccount(15000)
    ...
    .catch(function (err) {
      ...
      if (!kocAuthResolved) window.location.replace(DP_LOGIN_PATH);
    });
}, 1800);
