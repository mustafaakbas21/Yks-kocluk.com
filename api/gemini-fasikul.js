 ...
 *
 * Vercel: Proje ayarları → Environment Variables → GEMINI_API_KEY
 * Yerel: `.env` içine GEMINI_API_KEY=... yazıp `npx vercel dev` çalıştırın.
 *
 * İstemci: POST /api/gemini-fasikul (Google generateContent ile aynı JSON gövdesi)
 */
...
module.exports = async function handler(req, res) {
  ...
  if (!key || !String(key).trim()) {
    console.error("[gemini-fasikul] GEMINI_API_KEY tanımlı değil");
  ...
  try {
    ...
    console.error("[gemini-fasikul]", err);
