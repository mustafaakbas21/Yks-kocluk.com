...
```
yks-kocluk/
├── login.html        # E-posta / şifre girişi
...
**Önemli:** `koc-panel.html` ve `login.html` dosyalarını **HTTP üzerinden** açın (Live Server, `npx serve` vb.). `file://` ile ES modülleri ve Firebase sorun çıkarabilir.

1. **Giriş (test):** `login.html` — kullanıcı adı `admin`, şifre `admin123`. Oturum `localStorage` ile tutulur.
